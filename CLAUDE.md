...
- Phase 0 provenance/adjudication substrate: `make bench` emits `_meta.input_hashes` for the loaded project state, corpus, and supplied benchmark files; `make validate-adjudication-protocol PROTOCOL=protocol.json` checks pre-registered protocol metadata; `make adjudication-sample` exports unlabeled schema_version=1 sample packets for human/expert review inputs; `make validate-adjudication-responses PACKAGE=sample.json` validates completed response package shape/completeness; `make import-adjudication-responses ...` converts valid completed responses into D3/D7 gold package inputs when explicitly requested; `make write-phase0-adjudication-package ...` writes a strict Phase 0 manifest for those versioned package inputs. Hashes, protocol packages, sample packets, response validation, import files, and manifest assembly are reproducibility/protocol metadata, not validity evidence by themselves.

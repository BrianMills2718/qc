...
- **Observed pattern substrate**: cross-interview consensus, divergent-code, and code-co-occurrence outputs now become first-class `ObservedPattern` records with code/doc/application scope and default `causal_interpretation_status="descriptive_only"`. These records are descriptive pattern accounting and future abductive/workbench inputs, not causal proof or abductive synthesis by themselves.

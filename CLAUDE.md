...
- Graph viz, JSON/CSV/Markdown/QDPX export, optional CLI/MCP export-audit hash manifests, local verification, strict local publish/handoff preflight (`make export-publish-preflight`), and opt-in local hash-linked export-audit event logs (`AUDIT_LOG=...`, `make verify-export-audit-log`), per-stage memos, per-code audit trail; typed `PipelineContext`/results, fail-loud inter-stage checks, `llm_client` observability.

...
- D9 interpretive-preference protocol/scorecard substrate: `make validate-d9-interpretive-preference-protocol PROTOCOL=protocol.json` validates pre-evaluation blind preference protocol metadata for corpus/state/comparison-artifact hashes, blinding, evaluator plan, target criteria/surfaces, held-out gates, non-inferiority margin, and success criteria; `make bench PREFERENCE=preference.json` scores externally supplied forced-choice outcomes. These are protocol/accounting surfaces only, not blind expert-parity evidence, interpretive-depth evidence, methodological-validity evidence, or SOTA evidence.

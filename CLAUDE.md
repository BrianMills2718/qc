...
- GT constant comparison over the canonical char-anchored segment universe; category-saturation diagnostics, diagnostic-driven sampling suggestions, theoretical-sampling protocol validation/preflight, candidate/result package export, explicit repo-local candidate export, and a committed synthetic theoretical-sampling smoke artifact separate from codebook stability (INV-4 partial); incremental re-coding via `project recode` or explicit `project add-docs --recode` (hard-invalidates stale higher-order outputs by default, with per-project `auto_refresh_higher_order_on_recode` and one-run `--refresh-higher-order` / `--no-refresh-higher-order` overrides for thematic Perspective/Relationship/Synthesis and GT Axial/Selective/Theory Integration; INV-11 partial).
...
- **Observed pattern substrate**: cross-interview consensus, divergent-code, and code-co-occurrence outputs now become first-class `ObservedPattern` records with code/doc/application scope and default `causal_interpretation_status="descriptive_only"`. These records are inspectable through `project patterns`, `/projects/{project_id}/patterns`, and Markdown reports. They are descriptive pattern accounting and future abductive/workbench inputs, not causal proof or abductive synthesis by themselves.
- **Opt-in abductive candidate explanations**: `project run --abductive` inserts an abductive synthesis stage after cross-interview analysis and before negative-case analysis. It writes provisional `AbductiveCandidateExplanation` records with source pattern IDs, rival explanations, observable implications, and evidence gaps, and marks referenced patterns `candidate_explanation_generated`. Candidates are inspectable through `project abductive`, `/projects/{project_id}/abductive-explanations`, and Markdown reports; `ReviewManager` and `/projects/{project_id}/review/abductive-candidates` expose first-class candidate review targets, and `target_type="abductive_candidate"` decisions can approve candidates for evidence review, reject them, or modify bounded explanatory fields through the shared review-decision path. `make export-process-tracing-handoff` / `qc_cli.py export-process-tracing-handoff` write a strict schema_version=1 QC-side handoff package with scope, document hashes, observed patterns, abductive candidates, claims, anchors, provenance, and caveats. Process-tracing consumer review on 2026-06-25 accepted this as a QC-side fixture/adaptor input that can map to a future PT/workbench adapter without importing `pt.schemas` into QC and without adding likelihood/posterior/comparative-support fields. It is not a runnable PT input by itself; PT/workbench still owns the research-design/source-packet overlay (`research_question`, focal window, outcome, source candidates, known gaps, and pre-specified tests). These are hypotheses and boundary artifacts for review/process-tracing handoff, not causal proof, process-tracing results, methodological-validity evidence, or SOTA evidence.
...
- Phase 0 provenance/adjudication substrate: `make bench` emits `_meta.input_hashes` for the loaded project state, corpus, and supplied benchmark files; `make bench ARTIFACT_DIR=...` writes local `scorecard.json` / `timing_d10.json` / `manifest.json` artifact packages, and `make verify-phase0-benchmark-artifact ARTIFACT=...` verifies their local hashes and metadata consistency; `make validate-adjudication-protocol PROTOCOL=protocol.json` checks pre-registered protocol metadata; `make adjudication-protocol-preflight PROTOCOL=protocol.json SAMPLE=sample.json` checks that a concrete sample package matches that protocol before labeling; `make adjudication-sample PROJECTS_DIR=projects` exports unlabeled schema_version=1 sample packets for human/expert review inputs from an explicit project store when needed; `make validate-adjudication-responses PACKAGE=sample.json` validates completed response package shape/completeness; `make adjudication-response-preflight PROTOCOL=protocol.json SAMPLE=sample.json RESPONSES=responses.json` checks completed responses against protocol/sample provenance before import; `make import-adjudication-responses ... PREFLIGHT_PROTOCOL=protocol.json PREFLIGHT_SAMPLE=sample.json` converts valid completed responses into D3/D7 gold package inputs with an optional import-time preflight guard when explicitly requested; `make write-phase0-adjudication-package ...` writes a strict Phase 0 manifest for those versioned package inputs. Hashes, artifact verification, protocol packages, preflight reports, sample packets, response validation, response preflight, guarded import files, and manifest assembly are reproducibility/protocol metadata, not validity evidence by themselves.
...
python qc_cli.py project patterns <project_id> --limit 20 --offset 0
python qc_cli.py project patterns <project_id> --show-anchors
python qc_cli.py project abductive <project_id> --limit 20 --offset 0
python qc_cli.py export-process-tracing-handoff <project_id> --output handoff.json
python qc_cli.py validate-process-tracing-handoff handoff.json
...
python qc_cli.py project --projects-dir projects adjudication-sample <project_id> --output-file sample.json
...
3. **GT segment substrate unified**: `gt_constant_comparison.py` now consumes the canonical char-anchored `Segment` universe from `core/segmentation.py` through a compatibility wrapper. GT populates `ProjectState.segments` when absent and preserves existing canonical segments when present, so GT traversal, coverage/review/export surfaces, and future abductive evidence bundles share one denominator. Remaining follow-up: decide whether canonical non-speaker paragraph segmentation needs optional long-chunk policy for very large paragraphs; that belongs in `segment_corpus()`, not a second GT segmenter.
...
make compare-inv7-packages PACKAGES="inv7_a.json inv7_b.json" OUTPUT=inv7_compare.json  # Compare schema_version=1 INV-7 package outcomes
...
python qc_cli.py compare-inv7-packages inv7_a.json inv7_b.json --output inv7_compare.json  # Canonical CLI wrapper for INV-7 package comparison

...
- D6 bias protocol/scorecard substrate: `make validate-d6-bias-protocol PROTOCOL=protocol.json` validates pre-run D6 bias-audit protocol metadata; `make bench` can ingest externally supplied counterfactual identity-swap outcomes via `BIAS_COUNTERFACTUAL=bias.json` and externally supplied stratified correctness/error rows via `BIAS_STRATIFIED=bias_stratified.json`; both are local process/accounting surfaces only, not populated bias-audit evidence, causal proof, or a bias-free claim.

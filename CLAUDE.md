...
- GT constant comparison; category-saturation diagnostics, diagnostic-driven sampling suggestions, theoretical-sampling protocol validation/preflight, and loaded-document candidate export separate from codebook stability (INV-4 partial); incremental re-coding via `project recode` (hard-invalidates stale higher-order outputs it cannot recompute, INV-11 partial).

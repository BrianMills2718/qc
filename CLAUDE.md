...
- Phase 0 provenance/adjudication substrate: `make bench` emits `_meta.input_hashes` for the loaded project state, corpus, and supplied benchmark files; `make adjudication-sample` exports unlabeled schema_version=1 sample packets for human/expert review inputs. Hashes and sample packets are reproducibility/protocol metadata, not validity evidence.

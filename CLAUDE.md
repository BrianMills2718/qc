...
- Phase 0 provenance/adjudication substrate: `make bench` emits `_meta.input_hashes` for the loaded project state, corpus, and supplied benchmark files; `make adjudication-sample` exports unlabeled schema_version=1 sample packets for human/expert review inputs; `make validate-adjudication-responses PACKAGE=sample.json` validates completed response package shape/completeness before any later label import. Hashes, sample packets, and response validation are reproducibility/protocol metadata, not validity evidence.

...
bench:  ## Evaluation-harness Phase 0 scorecard (ID=<project_id> [PROJECTS_DIR=path] [D3_GOLD=d3_gold.json] [D3_BASELINES=d3_baselines.json] [D3_PROTOCOL=protocol.json] [GOLD=gold.json] [BASELINES=baselines.json] [PROMPT_INJECTION=inv7.json] [INV7_PROTOCOL=protocol.json] [D6_PROTOCOL=protocol.json] [D4_PROTOCOL=protocol.json] [D8_PROTOCOL=protocol.json] [D9_PROTOCOL=protocol.json] [CONFIDENCE_PROTOCOL=protocol.json] [BIAS_COUNTERFACTUAL=bias.json] [BIAS_STRATIFIED=bias_stratified.json] [CODEBOOK_QUALITY=quality.json] [GT_FIDELITY=gt_fidelity.json] [PREFERENCE=preference.json] [CALIBRATION=calibration.json] [OBS_DB=path] [TRACE_ID=id] [ARTIFACT_DIR=benchmark_results])
	python scripts/bench_phase0.py $(ID) $(if $(PROJECTS_DIR),--projects-dir $(PROJECTS_DIR),) $(if $(D3_GOLD),--d3-gold-file $(D3_GOLD),) $(if $(D3_BASELINES),--d3-baselines-file $(D3_BASELINES),) $(if $(D3_PROTOCOL),--d3-comparison-protocol-file $(D3_PROTOCOL),) $(if $(GOLD),--gold-file $(GOLD),) $(if $(BASELINES),--d7-baselines-file $(BASELINES),) $(if $(PROMPT_INJECTION),--prompt-injection-file $(PROMPT_INJECTION),) $(if $(INV7_PROTOCOL),--inv7-live-protocol-file $(INV7_PROTOCOL),) $(if $(D6_PROTOCOL),--d6-bias-protocol-file $(D6_PROTOCOL),) $(if $(D4_PROTOCOL),--d4-codebook-quality-protocol-file $(D4_PROTOCOL),) $(if $(D8_PROTOCOL),--d8-gt-fidelity-protocol-file $(D8_PROTOCOL),) $(if $(D9_PROTOCOL),--d9-interpretive-preference-protocol-file $(D9_PROTOCOL),) $(if $(CONFIDENCE_PROTOCOL),--confidence-calibration-protocol-file $(CONFIDENCE_PROTOCOL),) $(if $(BIAS_COUNTERFACTUAL),--bias-counterfactual-file $(BIAS_COUNTERFACTUAL),) $(if $(BIAS_STRATIFIED),--bias-stratified-file $(BIAS_STRATIFIED),) $(if $(CODEBOOK_QUALITY),--codebook-quality-file $(CODEBOOK_QUALITY),) $(if $(GT_FIDELITY),--gt-fidelity-file $(GT_FIDELITY),) $(if $(PREFERENCE),--interpretive-preference-file $(PREFERENCE),) $(if $(CALIBRATION),--confidence-calibration-file $(CALIBRATION),) $(if $(OBS_DB),--observability-db $(OBS_DB),) $(if $(TRACE_ID),--trace-id $(TRACE_ID),) $(if $(ARTIFACT_DIR),--artifact-dir $(ARTIFACT_DIR),)
...
compare-inv7-packages:  ## Compare INV-7 package outcomes (PACKAGES="a.json b.json" [OUTPUT=report.json])
ifndef PACKAGES
...
validate-sampling-frame-adequacy-protocol:  ## Validate a sampling-frame adequacy protocol (PROTOCOL=protocol.json)
ifndef PROTOCOL
	$(error PROTOCOL is required. Usage: make validate-sampling-frame-adequacy-protocol PROTOCOL=protocol.json)
endif
	python scripts/validate_sampling_frame_adequacy_protocol.py $(PROTOCOL)

sampling-frame-adequacy-preflight:  ## Preflight sampling-frame adequacy results against a protocol (PROTOCOL=protocol.json ADEQUACY=adequacy.json)
ifndef PROTOCOL
	$(error PROTOCOL is required. Usage: make sampling-frame-adequacy-preflight PROTOCOL=protocol.json ADEQUACY=adequacy.json)
endif
ifndef ADEQUACY
	$(error ADEQUACY is required. Usage: make sampling-frame-adequacy-preflight PROTOCOL=protocol.json ADEQUACY=adequacy.json)
endif
	python scripts/preflight_sampling_frame_adequacy_protocol.py $(PROTOCOL) --adequacy-file $(ADEQUACY)
...
ifndef GOLD
	$(error GOLD is required. Usage: make write-d7-comparison-package ID=<project_id> GOLD=gold.json PREDICTIONS="a.json b.json" OUTPUT=d7_package.json)
endif
ifndef PREDICTIONS
	$(error PREDICTIONS is required. Usage: make write-d7-comparison-package ID=<project_id> GOLD=gold.json PREDICTIONS="a.json b.json" OUTPUT=d7_package.json)
endif
ifndef OUTPUT
...
compare-d7-package:  ## Run D7 comparison from a strict package manifest (PACKAGE=d7_comparison_package.json)
ifndef PACKAGE
	$(error PACKAGE is required. Usage: make compare-d7-package PACKAGE=d7_comparison_package.json)
endif
	python scripts/run_d7_comparison_package.py $(PACKAGE)

validate-d3-comparison-protocol:  ## Validate a D3 baseline comparison protocol (PROTOCOL=protocol.json)
ifndef PROTOCOL
	$(error PROTOCOL is required. Usage: make validate-d3-comparison-protocol PROTOCOL=protocol.json)
endif
	python scripts/validate_d3_comparison_protocol.py $(PROTOCOL)

d3-comparison-preflight:  ## Preflight D3 comparison inputs (PROTOCOL=protocol.json GOLD=d3_gold.json PREDICTIONS="a.json b.json")
ifndef PROTOCOL
	$(error PROTOCOL is required. Usage: make d3-comparison-preflight PROTOCOL=protocol.json GOLD=d3_gold.json PREDICTIONS="a.json b.json")
endif
ifndef GOLD
	$(error GOLD is required. Usage: make d3-comparison-preflight PROTOCOL=protocol.json GOLD=d3_gold.json PREDICTIONS="a.json b.json")
endif
ifndef PREDICTIONS
	$(error PREDICTIONS is required. Usage: make d3-comparison-preflight PROTOCOL=protocol.json GOLD=d3_gold.json PREDICTIONS="a.json b.json")
endif
	python scripts/preflight_d3_comparison.py $(PROTOCOL) $(GOLD) $(PREDICTIONS)

...
bench:  ## Evaluation-harness Phase 0 scorecard (ID=<project_id> [D3_GOLD=d3_gold.json] [D3_BASELINES=d3_baselines.json] [GOLD=gold.json] [BASELINES=baselines.json] [PROMPT_INJECTION=inv7.json] [D6_PROTOCOL=protocol.json] [D4_PROTOCOL=protocol.json] [BIAS_COUNTERFACTUAL=bias.json] [BIAS_STRATIFIED=bias_stratified.json] [CODEBOOK_QUALITY=quality.json] [GT_FIDELITY=gt_fidelity.json] [PREFERENCE=preference.json] [CALIBRATION=calibration.json] [OBS_DB=path] [TRACE_ID=id] [ARTIFACT_DIR=benchmark_results])
	python scripts/bench_phase0.py $(ID) $(if $(D3_GOLD),--d3-gold-file $(D3_GOLD),) $(if $(D3_BASELINES),--d3-baselines-file $(D3_BASELINES),) $(if $(GOLD),--gold-file $(GOLD),) $(if $(BASELINES),--d7-baselines-file $(BASELINES),) $(if $(PROMPT_INJECTION),--prompt-injection-file $(PROMPT_INJECTION),) $(if $(D6_PROTOCOL),--d6-bias-protocol-file $(D6_PROTOCOL),) $(if $(D4_PROTOCOL),--d4-codebook-quality-protocol-file $(D4_PROTOCOL),) $(if $(BIAS_COUNTERFACTUAL),--bias-counterfactual-file $(BIAS_COUNTERFACTUAL),) $(if $(BIAS_STRATIFIED),--bias-stratified-file $(BIAS_STRATIFIED),) $(if $(CODEBOOK_QUALITY),--codebook-quality-file $(CODEBOOK_QUALITY),) $(if $(GT_FIDELITY),--gt-fidelity-file $(GT_FIDELITY),) $(if $(PREFERENCE),--interpretive-preference-file $(PREFERENCE),) $(if $(CALIBRATION),--confidence-calibration-file $(CALIBRATION),) $(if $(OBS_DB),--observability-db $(OBS_DB),) $(if $(TRACE_ID),--trace-id $(TRACE_ID),) $(if $(ARTIFACT_DIR),--artifact-dir $(ARTIFACT_DIR),)

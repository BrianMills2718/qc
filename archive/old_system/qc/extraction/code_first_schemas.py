...
class EnhancedQuote(BaseModel):
    ...
    # Thematic connection fields (for focus groups)
    thematic_connection: Optional[str] = None  # "builds_on", "challenges", "clarifies", "supports", "responds_to", "contradicts", "none"
    connection_target: Optional[str] = None    # Speaker name being connected to
    connection_confidence: Optional[float] = None  # 0.0-1.0 confidence score
    connection_evidence: Optional[str] = None      # Brief phrase showing connection
...
class SimpleQuote(BaseModel):
    ...
    sequence_position: Optional[int] = None  # Position in conversation sequence (for focus groups)
...
class QuotesAndSpeakers(BaseModel):
    ...
    total_quotes: int
    total_codes_applied: int
    thematic_connections_detected: int = 0  # Count of quotes with thematic connections (focus groups)
    ...
    def model_post_init(self, __context):
        ...
        if self.thematic_connections_detected == 0 and self.quotes:
            # Auto-calculate from quotes with thematic connections

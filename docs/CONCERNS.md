...
| QC-WB-001 | Future `mixed_methods_workbench` / `process_tracing` alignment can drift if QC exports untyped JSON. | Medium | Plan #233 adds a strict schema_version=1 QC-side handoff package and validator while keeping process-tracing inference outside this repo. | Request process-tracing consumer review of the deterministic handoff fixture before treating the seam as stable. |

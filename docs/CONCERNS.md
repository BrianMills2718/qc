...
| QC-PORT-010 | Sanitization/de-identification is needed before any public/shareable corpus claim. | Medium | Plan #234 keeps the Africa transcript work local and records sanitization as deferred, while raw files remain ignored and uncommitted. | After the local corpus/adjudication seed works, design an agent-drivable sanitizer with inventory, identifier detection, reviewed redaction/generalization, hashing, and residual-risk metadata. |

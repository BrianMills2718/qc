...
- **QC surface:** current surfaces are `qc_cli.py bench <project_id>` and `make bench ID=<project_id>` for the deterministic Phase 0 local scorecard, with optional `D3_GOLD=d3_gold.json` / `--d3-gold-file`, `GOLD=gold.json` / `--gold-file`, `BASELINES=baselines.json` / `--d7-baselines-file`, `PROMPT_INJECTION=inv7.json` / `--prompt-injection-file`, `BIAS_COUNTERFACTUAL=bias.json` / `--bias-counterfactual-file`, `OBS_DB=` / `--observability-db` plus `TRACE_ID=` / `--trace-id` overrides for D10 cost/latency, and `ARTIFACT_DIR=benchmark_results` / `--artifact-dir` for a versioned Phase 0 package. `make run-inv7-fixtures` and `make run-inv7-live-fixtures` can produce structural or live INV-7 input files for `PROMPT_INJECTION=`. The scorecard includes `_meta.input_hashes` for the loaded project state, corpus, and supplied benchmark files. Target future behavior behind `qc bench` is a frozen `prompt_eval` suite against a dataset that writes a scored report. Agent-drivable end to end (no manual steps).

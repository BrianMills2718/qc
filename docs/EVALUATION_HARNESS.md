...
> and D10 cost/latency from observability rows; deterministic, LLM-free). **Not yet built:** there is
...
- **D10 cost/latency:** **scoring substrate implemented** — `make bench` reports `cost_latency_d10` from real `llm_client` `llm_calls` rows. Default matching uses the local project trace prefix (`qualitative_coding/project/<project_id>`), with `TRACE_ID=` / `--trace-id` for exact non-default traces and `OBS_DB=` / `--observability-db` for DB path overrides. Missing DB or matching rows reports `not_available`; costs are never estimated from `ProjectState`. The latency value is summed observed LLM-call latency, not full pipeline wall-clock time.
...
- **Phase 0 local scorecard — DONE.** `make bench ID=<project>` (`qc_clean/core/bench.py` → `phase0_scorecard`) computes **D1 grounding** (now real: `verify_grounding` returns the fraction of applications whose span anchor verifies — INV-1 landed), **D2 coverage** (including examined-and-judged coverage under `--exhaustive`), surfaces **D5** reliability/stability when present, includes a gold-dependent **D7 disconfirmation** exact-anchor score plus recall/precision Wilson intervals when adjudicated gold contrary evidence is present in project metadata or supplied through `GOLD=gold.json`, can score externally supplied D7 baseline predictions through `BASELINES=baselines.json`, reports externally supplied **INV-7 prompt-injection** fixture outcomes when supplied through `PROMPT_INJECTION=inv7.json`, and reports **D10 cost/latency** from matching `llm_client` observability rows. Deterministic and LLM-free. Remaining harness work: wire into `prompt_eval` for frozen case sets + bootstrap/F1 intervals, build/run held-out D3/D7 gold sets, run actual baselines, run live adversarial prompt-injection fixtures, add full wall-clock run timing, and run on reused public gold as a comparator.

...
- **QC surface:** current surfaces are `qc_cli.py bench <project_id>` and `make bench ID=<project_id>` for the deterministic Phase 0 local scorecard, with optional `D3_GOLD=d3_gold.json` / `--d3-gold-file`, `GOLD=gold.json` / `--gold-file`, `BASELINES=baselines.json` / `--d7-baselines-file`, `PROMPT_INJECTION=inv7.json` / `--prompt-injection-file`, `BIAS_COUNTERFACTUAL=bias.json` / `--bias-counterfactual-file`, `CODEBOOK_QUALITY=quality.json` / `--codebook-quality-file`, `OBS_DB=` / `--observability-db` plus `TRACE_ID=` / `--trace-id` overrides for D10 cost/latency, and `ARTIFACT_DIR=benchmark_results` / `--artifact-dir` for a versioned Phase 0 package. `make run-inv7-fixtures` and `make run-inv7-live-fixtures` can produce structural or live INV-7 input files for `PROMPT_INJECTION=`. The scorecard includes `_meta.input_hashes` for the loaded project state, corpus, and supplied benchmark files. Target future behavior behind `qc bench` is a frozen `prompt_eval` suite against a dataset that writes a scored report. Agent-drivable end to end (no manual steps).
...
- **Phase 0 local scorecard — DONE.** `qc_cli.py bench <project>` / `make bench ID=<project>` (`qc_clean/core/bench.py` → `phase0_scorecard`) computes **D1 grounding** (now real: `verify_grounding` returns the fraction of applications whose span anchor verifies — INV-1 landed), **D2 coverage** (including examined-and-judged coverage under `--exhaustive`), surfaces externally supplied **D4 codebook-quality** rubric outcomes through `CODEBOOK_QUALITY=quality.json`, surfaces **D5** reliability/stability with Gwet's AC1, prevalence tables, and local row-bootstrap intervals when present, includes a gold-dependent exact code/source-anchor **D3 application-validity** score with recall/precision Wilson intervals, F1 bootstrap intervals, local same-code/doc span-IoU plus Modified Hausdorff diagnostics, and human-ceiling exact-metric comparison plus human-human κ/α/AC1 metadata when supplied through a versioned `D3_GOLD=d3_gold.json` package, includes an externally supplied **D6 counterfactual bias** scorecard through `BIAS_COUNTERFACTUAL=bias.json`, includes a gold-dependent **D7 disconfirmation** exact-anchor score with recall/precision Wilson intervals, F1 bootstrap intervals, local paired baseline-delta bootstrap intervals, and human-ceiling exact-metric comparison plus human-human κ/α/AC1 metadata when supplied through a versioned `GOLD=gold.json` package, can score externally supplied D7 baseline predictions through `BASELINES=baselines.json`, reports externally supplied **INV-7 prompt-injection** fixture outcomes when supplied through `PROMPT_INJECTION=inv7.json`, reports **D10 cost/latency** from matching `llm_client` observability rows, reports **D10 wall-clock runtime** from the last local `project run`, includes `_meta.input_hashes` for project/corpus/external inputs, and can write a versioned Phase 0 `benchmark_results/` package containing `scorecard.json` plus manifest. Deterministic and LLM-free unless a separate opt-in live INV-7 fixture file is generated before scoring. Remaining harness work: wire into `prompt_eval` for frozen case sets beyond the local exact-key bootstrap, build/run held-out D3/D7 gold sets, run D4 LLM-judge and blind expert-panel evaluations, add system D3 κ/α agreement-vs-gold and populated human-ceiling comparison data, run a populated prompt_eval-backed D6 bias suite, run actual/live baselines on held-out data, run and score live adversarial prompt-injection fixtures against a committed benchmark protocol, turn public benchmark timing into full benchmark artifacts, and run on reused public gold as a comparator.

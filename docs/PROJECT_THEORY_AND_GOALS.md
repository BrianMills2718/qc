...
> not crashes); INV-9 first-class claim ledger object layer landed. Statuses
> below reflect this.
...
- **INV-6 — Disconfirmation covers the *final* claim set.** Every output-producing stage's claims must be subject to disconfirmation — including the last one. *Status: **PARTIAL** (was over-marked MET in v3.0 — corrected here per INV-0).* `NegativeCaseStage` now runs **last** and ingests the codebook **and** the cross-interview `cross_case` memo (locked by `tests/test_pipeline_engine.py` + `tests/test_negative_case_inv6.py`). **But it does NOT yet challenge the full `ProjectState.claims` ledger** — `synthesis`, `perspective_analysis`, entities/relationships, GT categories/propositions, and negative-case `overall_assessment` are not ledger-wide disconfirmation targets. Now that INV-9's object layer exists, the next step is to drive disconfirmation over the ledger; until then, do **not** say "all final claims survived disconfirmation."
...
| "Experimental disconfirmation over the codebook + cross-interview claims" | "Negative-case analysis establishes credibility" / "all final claims were disconfirmed" (the full ledger is not yet a disconfirmation target; INV-6) |
...
- **Claim ledger existence ≠ claim validity** (INV-9 mostly met; INV-6/10 still partial/unmet). Substantive assertions now become typed claim objects, but many higher-order claims still need source anchors, ledger-wide disconfirmation, and human adjudication. A polished ledger row can still be unsupported or provisional.
...
5. **Disconfirmation + adjudication over the ledger** — **NEXT**: extend negative-case search and human review to applications/claims/relations/negative-cases. *Closes INV-6 fully and INV-10*; hardened retrieval-first, different-model disconfirmation *closes INV-2*.

...
> outcomes can be scored when externally supplied, external held-out live
> fixture manifests can be run under protocol/preflight guards, committed
> held-out live artifacts exist, and multiple INV-7 result packages can be
> compared deterministically; INV-4 theoretical-sampling package workflow now
> has a committed synthetic repo-local smoke artifact; sampling-frame adequacy
> has protocol/result preflight infrastructure for future reviewer packages.
> Statuses below reflect this.
...
- **`observed_patterns: List[ObservedPattern]`** — first-class descriptive pattern records produced from deterministic stage outputs. Current producers: Cross-Interview consensus, divergent-code, and code-co-occurrence records. They are inspectable through CLI `project patterns`, API `/projects/{project_id}/patterns`, and Markdown report output. These are descriptive pattern accounting objects with `causal_interpretation_status="descriptive_only"` by default; they are not causal proof or abductive explanations.
- **`abductive_explanations: List[AbductiveCandidateExplanation]`** — opt-in provisional candidate explanations produced from observed patterns when `project run --abductive` is used. Each candidate records source pattern IDs, a mechanism summary, rival explanations, observable implications, and evidence gaps, and can be inspected through CLI `project abductive`, API `/projects/{project_id}/abductive-explanations`, and Markdown report output. Candidates are reviewable through `target_type="abductive_candidate"` and can be included in a strict schema_version=1 QC-to-process-tracing handoff package with patterns, claims, anchors, scope, hashes, and caveats. These are hypotheses and boundary artifacts for review and future process-tracing handoff, not causal proof, process-tracing results, methodological-validity evidence, or SOTA evidence.
...
**Incremental** (`incremental_coding.py`) — `project recode`: codes only
*uncoded* documents against the existing codebook, merges, then re-runs
downstream state-driven stages (Cross-Interview → Negative Case).
`project add-docs --recode` explicitly adds documents and invokes that same
recode path in one command; plain `add-docs` does not silently spend model
budget. Default recode deliberately omits stages that depend on ctx fields from
a coding stage that is not re-run (Perspective/Relationship/Synthesis,
Axial/Selective/Theory). **Staleness handling (INV-11):** default recode clears
populated stale outputs (`synthesis`, `perspective_analysis`,
`code_relationships`, `entities`, `entity_relationships`, `core_categories`,
`theoretical_model`), removes stale phase results and stale claim-ledger rows
from the invalidated source stages, and appends a `data_warnings` entry naming
what was invalidated. `project recode --refresh-higher-order` and
`project add-docs --recode --refresh-higher-order` force methodology-specific
refresh for one run; `--no-refresh-higher-order` forces hard-invalidation mode
for one run. `ProjectConfig.auto_refresh_higher_order_on_recode`, surfaced by
`project create --auto-refresh-higher-order-on-recode` and
`project recode-policy`, makes that refresh the project default without
flipping the global default for existing projects. Default/thematic projects
reconstruct Phase 1 JSON from the current codebook and rerun Perspective →
Relationship → Synthesis; grounded-theory projects reconstruct GT open-code
context from the current codebook and rerun Axial → Selective → Theory
Integration; both then rerun Cross-Interview → Negative Case. INV-11 remains
partial because the global/default policy is still hard invalidation unless a
project opts in.
...
`ReviewManager` (`review.py`) implements the operations a qualitative reviewer needs for code/codebook review, over CLI (`project review`) and a browser UI (`/review/<id>`):
...
Every decision is recorded as a `HumanReviewDecision` (action, target, rationale, new value) — the human audit trail — and updates code `provenance` to `HUMAN`. `ReviewManager` also supports `target_type="claim"` for approving, rejecting/withdrawing, or revising `AnalyticClaim` objects with `ClaimRevision` history; `target_type="code_relationship"` / `target_type="entity_relationship"` for bounded relationship review; and `target_type="abductive_candidate"` for approving provisional abductive candidates into `needs_evidence_review`, rejecting them, or modifying bounded explanatory fields. Browser-native abductive candidate review is still future work. Review raises `ValueError` on unknown target types/actions (fail loud). The pipeline can pause at review checkpoints when `enable_human_review=True`.
...
Export overwrite policy is explicit: `ProjectExporter` and `project export`
overwrite existing targets by default for backward compatibility, and
`project export --no-overwrite` / `overwrite=False` fail before writing if any
target artifact already exists. CSV no-overwrite checks the full artifact set
before writing, so it does not leave partial exports. This is local clobber
prevention only, not append-only or tamper-evident storage.
...
**Proven (software works; verified by deterministic tests + live E2E):** both pipelines run end-to-end; stages compose; schemas validate; failures are caught and localized; **span-anchored grounding** (char offsets + hash + verification, ambiguous/unresolvable dropped — INV-1 mostly met); a **char-anchored segment universe** + **coverage** denominator, with **exhaustive per-segment coding** (`--exhaustive`) giving examined-and-judged coverage (INV-8 met in that mode); a **first-class claim ledger object layer** (`ProjectState.claims`, stage source/kind/scope/support/adjudication fields, CLI/API/MCP/export read surfaces with scope and bounded anchor-detail inspection where applicable, plus API/MCP/CLI claim-ledger pagination/count metadata or display — INV-9 mostly met); optional **corpus-scope contract** (`ProjectState.corpus_scope`, surfaced in Markdown exports and CSV/Markdown claim-row context, missing/incomplete-scope warnings for claim-bearing Markdown reports plus JSON/CSV warning metadata, readable/updatable through CLI/API, arbitrary-text scope phrasing lint via `make lint-scope-phrasing`, and opt-in publish-preflight scope lint via `make export-publish-preflight ... SCOPE_LINT=1`, `project export --publish-preflight --scope-lint`, or MCP `qc_export_markdown(..., publish_preflight=True, scope_lint=True)`) for report boundary context; review (CLI + browser), adjudication protocol validation (`make validate-adjudication-protocol`), protocol-to-sample preflight (`make adjudication-protocol-preflight`), unlabeled adjudication sample export (`make adjudication-sample`), adjudication response shape/completeness validation (`make validate-adjudication-responses`), completed response protocol/sample preflight (`make adjudication-response-preflight`), conversion of valid completed adjudication responses into D3/D7 gold package inputs with optional import-time preflight guard inputs (`make import-adjudication-responses ... PREFLIGHT_PROTOCOL=... PREFLIGHT_SAMPLE=...`), strict Phase 0 manifest assembly for imported D3/D7 package inputs (`make write-phase0-adjudication-package`), optional export no-clobber guard (`project export --no-overwrite`), optional CLI/MCP export-audit hash manifests, local verification, strict local publish/handoff preflight, opt-in inline project-export preflight, opt-in MCP export preflight, opt-in local hash-linked event logs, and optional SQLite event-log mirrors (`make export-audit-manifest`, `make verify-export-audit-manifest`, `make export-publish-preflight`, `make verify-export-audit-log`, `make mirror-export-audit-db`, `make verify-export-audit-db`, `project export --audit-manifest --verify-audit-manifest --publish-preflight --audit-log --audit-db`, MCP `audit_manifest=True`, MCP `publish_preflight=True`, MCP `audit_event_log=True`, or MCP `audit_event_db=True`), IRR/stability, incremental recode, graph views, and JSON/CSV/Markdown/QDPX export function; ruff + docs gates green. This is **software/integration validation** — "the program does what it's built to do." It is *not* evidence the analysis is correct.
...
local same-target/doc char-span IoU overlap diagnostic, local discrete Modified
Hausdorff distance diagnostic, exact-key binary system-vs-gold percent
agreement/κ/AC1 plus prevalence,
optional baseline comparisons with local paired exact-key delta bootstrap
intervals and per-baseline same-target/doc span-overlap diagnostics,
human-ceiling exact-metric comparison, human-human κ/α/AC1 metadata
...
runners able to produce compatible input files, and committed built-in live
canary artifacts including current fixture-set v2 at
`docs/benchmarks/inv7_live_canary_v2_2026_06_22/`, an **externally supplied D8
...
independently curated/model-provider adversarial prompt-injection robustness benchmark, public
...
bootstrap intervals plus per-baseline same-code/document span-overlap
diagnostics. Versioned schema_version=1 D3 baseline packages validate through
`make validate-d3-baseline-package PACKAGE=...`, and recognized versioned
packages validate before scoring while legacy raw inputs remain supported.
`make validate-d3-comparison-protocol PROTOCOL=protocol.json` validates pre-run
D3 comparison metadata and optional exact/span metric criteria, and
`make d3-comparison-preflight ...` cross-checks a registered protocol against
versioned D3 gold and baseline packages before scoring; top-level
`qc_cli.py validate-d3-comparison-protocol` and
`qc_cli.py d3-comparison-preflight` delegate to those same canonical scripts.
`make bench
D3_PROTOCOL=... D3_GOLD=... D3_BASELINES=...` enforces that preflight before
scorecard/output/artifact writes and records
`_meta.preflight_reports.d3_comparison`; when protocol metric criteria are
present, it also records `application_validity_d3.metric_criteria_report`
pass/fail/missing rows. This is package/provenance validation, local accounting,
local error-analysis metadata, protocol metadata, preflight metadata,
score-time guard metadata, and metric-criteria metadata, not a live baseline
run, held-out D3 result, expert-parity claim, superiority claim, or SOTA claim.
...
**Planned / not yet done (do not imply these exist):** any **methodological validity** evidence (blind expert ratings, gold-standard agreement from real expert labels, populated bias stratification/counterfactual audit); full **tamper-evident audit substrate** (append-only/signed log; export hash manifests, local verification, publish preflight, hash-linked event logs, and SQLite event-log mirrors are only local integrity metadata); semantic/adversarial/human-adjudicated disconfirmation with D7 validation (INV-2); populated expert adjudication runs; browser-native/full human claim-review workflow; true theoretical sampling, per-category saturation, full axial decomposition, multi-model consensus, active learning, collaborative coding, retrieval grounding.
...
- **INV-2 — Source-anchored, retrieval-first disconfirmation.** Negative cases must be drawn by retrieving candidate contrary passages from the corpus *first*, then interpreting — ideally with a different model/adversarial prompt — each anchored (INV-1) and human-adjudicated **before the claim set is treated as final**. *Status: **PARTIAL (lexical/query-expanded retrieval-first + opt-in embedding-hybrid retrieval + adversarial routing + D7 scoring/export/baseline-validation/comparison/protocol-preflight/guard/metric-criteria/report-provenance/artifact-package/artifact-verifier/package-writer/package-runner substrate slices).*** `qc_clean/core/disconfirmation.py` retrieves bounded char-anchored segment candidates for live claim-ledger targets before LLM interpretation using BM25-style term weighting, configurable contradiction-cue boosts, deterministic query-expansion terms, and an opt-in `embedding_hybrid` mode that combines lexical score with embedding cosine similarity when an embedding model is explicitly configured; `NegativeCaseStage` sends retrieved candidates instead of the full corpus, frames interpretation as skeptical/adversarial evidence review, and can route through `PipelineContext.disconfirmation_model_name`; `NegativeCase.candidate_id` lets returned cases attach exact candidate anchors, invalid candidate IDs fail loud, and API/MCP/browser review rows expose bounded supporting/contrary anchor details for inspection; `make bench` computes exact D7 recall/precision/F1 plus 95% Wilson intervals for recall/precision, a local exact-key F1 bootstrap interval, local same-target/document span-IoU plus Modified Hausdorff diagnostics, local paired exact-key baseline-delta bootstrap intervals, and per-baseline span-overlap diagnostics when adjudicated contrary-evidence gold anchors and baseline predictions are present; `make run-d7-retrieval` exports retrieval-mode candidates as baseline-compatible predictions with project/corpus/state/trace/budget provenance, `make validate-d7-baseline-package` validates versioned retrieval/live-baseline prediction packages, recognized versioned `BASELINES=` files validate before scoring, `make validate-d7-comparison-protocol` validates registered comparison metadata plus optional structured metric criteria, `make d7-comparison-preflight` cross-checks versioned D7 gold and retrieval/live-baseline prediction packages against that protocol, `qc_cli.py validate-d7-comparison-protocol` and `qc_cli.py d7-comparison-preflight` delegate to those same canonical scripts, `make compare-d7-retrieval PROTOCOL=... ARTIFACT_DIR=...` can enforce the same preflight before scoring multiple exported packages against one gold file, report criteria pass/fail/missing status when criteria are present, record `_meta.input_hashes` / `_meta.command` provenance, and write an optional versioned D7 comparison artifact package, `make verify-d7-comparison-artifact` verifies that artifact's local manifest/report consistency, `make write-d7-comparison-package` can write a strict comparison manifest after validating versioned D7 gold and prediction packages plus optional protocol preflight, and `make compare-d7-package` can run the same comparison from that manifest. **Remaining gaps:** embedding-hybrid retrieval is opt-in and unvalidated; no validated default embedding/adversarial model/provider policy has been chosen; target/candidate counts are bounded; zero retrieved candidates is not evidence of no contrary evidence; candidate results are not human-adjudicated; no held-out D7 gold-set benchmark with live baselines or held-out retrieval-mode comparisons has run. D7 span-overlap diagnostics, metric-criteria reports, artifact manifests, artifact verification reports, package-writer reports, package-runner manifests, and anchor-detail review rows are local error-analysis/accounting/provenance/review-context metadata only, not semantic disconfirmation validity; the D7 baseline-package validation and comparison protocol/preflight/guard/report-provenance/artifact-package/verifier/writer/runner surfaces are provenance only. Disconfirmation is experimental and **not** credibility evidence.
...
- **INV-9 — First-class claim ledger.** Every substantive analytic assertion (codes, applications, perspective claims, entity relations, code relations, synthesis findings, GT propositions, negative cases) must be an explicit typed object with: source stage, claim text, scope, supporting anchors, contrary anchors, adjudication status, and revision history. *Status: **MOSTLY MET (object layer).*** `ProjectState.claims` stores `AnalyticClaim` objects; default/thematic and GT stages derive claims or no-claims events from validated domain outputs; negative cases can link challenged code/claim targets and contrary anchors; CSV/Markdown/CLI/API/MCP expose bounded ledger summaries; CLI/API/MCP claim read surfaces and API/MCP/browser review rows expose scope and bounded supporting/contrary anchor details for inspection; API/MCP claim-ledger responses support `limit`/`offset` traversal and report `returned`, `total_claims`, applied `limit`, and applied `offset`; CLI `project claims` supports `--limit`/`--offset` page display; `make bench` reports deterministic `claim_anchor_coverage` counts/rates/breakdowns for anchored versus unanchored claim rows. *Remaining gaps:* higher-order claims are often `needs_anchor` until explicit source-span support is added; scope visibility, anchor coverage, count metadata, pagination metadata, and anchor details are structural accounting/review context, not truth or human review; retrieval-first disconfirmation is bounded, embedding-hybrid retrieval is opt-in/unvalidated, and no held-out D7 result exists (INV-2/6); claim review exists in `ReviewManager` but not a full browser-native workflow (INV-10); prompt schemas still emit domain objects that are adapted into claims rather than claim-native outputs. Do not say the ledger proves every claim is supported, disconfirmed, or human-retained.
- **INV-10 — Adjudication beyond code labels.** Human review must reach applications, claims, relationships, negative cases, and provisional abductive candidates — not only code objects. *Status: **PARTIAL**.* `ReviewManager` supports code applications, `target_type="claim"` decisions for approve/reject/modify (updating `AnalyticClaim.adjudication_status` and `revision_history`), relationship decisions for explicit `target_type="code_relationship"` / `target_type="entity_relationship"` approve/reject/modify flows, and `target_type="abductive_candidate"` decisions that can approve candidates into `needs_evidence_review`, reject them without deleting history, or modify bounded explanatory fields; `ReviewSummary` includes `claims_count`, `relationships_count`, `abductive_candidates_count`, and active/inactive review-decision counters; `/projects/{id}/review/claims` and MCP `qc_review_claims` expose bounded, `limit`/`offset`-pageable claim review rows for API/browser and agent clients; `/projects/{id}/review/negative-cases` and MCP `qc_review_negative_cases` expose bounded, `limit`/`offset`-pageable negative-case claim rows using the same `target_type="claim"` decision contract; `/projects/{id}/review/relationships` and MCP `qc_review_relationships` expose bounded, `limit`/`offset`-pageable code/entity relationship review rows; `/projects/{id}/review/abductive-candidates` exposes bounded candidate rows for API/agent review; MCP `qc_review_decisions` can apply claim and relationship decisions with rationale for agent-drivable review, while generic API/CLI decision paths can apply abductive-candidate decisions. The browser review page has Codes, Claims, Negative Cases, and Relationships modes; browser-native abductive candidate review remains future UI work. Remaining gaps: sampled expert adjudication protocol (INV-3), held-out validity evidence, browser-native abductive candidate review, and richer sampled workflows over quote applications and downstream claims. A code label can be fine while its quote applications, downstream claims, or abductive hypotheses are wrong; marking the code human-reviewed must not launder unreviewed LLM inferences below it.
- **INV-11 — No stale higher-order outputs.** When the corpus changes (e.g. `project recode`), any downstream output not recomputed must be invalidated or flagged, never silently retained. *Status: **PARTIAL (explicit add-docs recode hook + hard-invalidation default + opt-in thematic/GT higher-order refresh + project-level refresh policy).*** `project add-docs --recode` can now perform corpus mutation and invoke incremental recode in one opt-in command; plain `add-docs` still does not silently spend model budget. Default `IncrementalCodingStage` clears stale higher-order outputs it does not recompute (`synthesis`, `perspective_analysis`, `code_relationships`, `entities`, `entity_relationships`, `core_categories`, `theoretical_model`), removes matching stale phase results and claim-ledger rows for invalidated source stages, marks review decisions targeting removed claims inactive while retaining them as historical audit records, and appends a `data_warnings` entry naming what was invalidated. That warning is surfaced on the human + agent surfaces that matter: CLI human formatter, API JSON, JSON export, **Markdown export** (prominent block), MCP `qc_recode`/`qc_get_synthesis`/`qc_get_codebook`, and graph code/entity data responses. `--refresh-higher-order` forces methodology-specific refresh for one run, `--no-refresh-higher-order` forces hard-invalidation mode for one run, and `ProjectConfig.auto_refresh_higher_order_on_recode` can make refresh the project default through `project create --auto-refresh-higher-order-on-recode` or `project recode-policy`. For default/thematic projects, refresh rebuilds Phase 1 JSON from the updated codebook and reruns Perspective, Relationship, and Synthesis before Cross-Interview and Negative Case. For grounded-theory projects, it rebuilds GT open-code context from the updated codebook and reruns Axial, Selective, and Theory Integration before Cross-Interview and Negative Case. *Remaining gap (why PARTIAL, not MET):* the global default still uses hard invalidation unless a project opts in, and no live-validated decision has made auto-refresh the universal corpus-mutation policy. Locked by `tests/test_incremental_staleness_inv11.py`, `tests/test_incremental.py`, and `tests/test_project_commands.py`.
...
- **Corpus boundary is recordable, not validated.** `ProjectState.corpus_scope` can now state phenomenon, population, sampling frame, inclusion/exclusion criteria, and caveats; CLI/API surfaces can read/update it, and CLI/MCP project creation can set it at creation time; Markdown export surfaces it before analytic claims, CSV/Markdown claim rows include compact claim-scope and corpus-boundary context, JSON/CSV exports emit machine-readable warnings when claim-bearing exports have no recorded scope, an empty scope record, or a population without a sampling frame, `make lint-scope-phrasing` can scan arbitrary text for risky population-generalizing phrases under missing/under-specified scope, `make export-publish-preflight ... SCOPE_LINT=1`, `qc_cli.py project export ... --publish-preflight --scope-lint`, and MCP `qc_export_markdown(..., publish_preflight=True, scope_lint=True)` can block risky Markdown/text handoff artifacts under missing/under-specified scope, `make bench` reports `corpus_scope_adequacy` status and field completeness, and `make validate-sampling-frame-adequacy-protocol` / `make sampling-frame-adequacy-preflight` can validate/preflight future reviewer result packages. That helps report discipline and governed evaluation setup; it does **not** prove the sampling frame is adequate or make population generalization safe. If scope is absent or under-specified, fall back to "these N transcripts," not population language.
Top-level `qc_cli.py lint-scope-phrasing` delegates to the same scope-language lint script, and `qc_cli.py export-publish-preflight --scope-lint` / `qc_cli.py project export --publish-preflight --scope-lint` / MCP `publish_preflight=True, scope_lint=True` delegate to the same opt-in publish-preflight gate; both are report-boundary discipline only, not sampling-frame adequacy evidence.
...
LLM bias may be systematic not random (INV-3/5); reliability ≠ validity; pseudo-replication; the denominator problem (INV-8 default path); claim ledger rows can be provisional/unanchored/unadjudicated (INV-9/10); GT fidelity partial ("GT-inspired," not "full GT"); disconfirmation is retrieval-first/adversarially framed now but still bounded, defaults to BM25-style lexical/query-expanded retrieval, has only opt-in unvalidated embedding-hybrid retrieval, is not held-out D7-validated, and remains experimental even with optional baseline point-comparison wiring (INV-2/6); prompt-injection scoring can ingest structural, built-in live canary, or external live fixture-manifest files, committed built-in canaries plus external held-out artifacts exist including a 10-fixture held-out live v1 package, and INV-7 packages can be compared deterministically, but these are bounded measurement/provenance outputs rather than prompt-injection robustness, model-obedience, model/provider superiority, methodological-validity, or SOTA evidence (INV-7); D10 wall-clock timing is last-local-run metadata and can be packaged as a local timing artifact with runtime environment metadata, but it is not public benchmark timing evidence; confidence uncalibrated; corpus scope is recordable/exported but not sampling-frame validation; incremental staleness is hard-invalidated by default unless a project enables configured higher-order auto-refresh or a run forces refresh, so universal default auto-refresh is still partial (INV-11); export hash manifests, verification, publish preflight, hash-linked event logs, and SQLite event-log mirrors are local integrity metadata, not a full tamper-evident audit substrate; methodological validity unmeasured; non-determinism inherent (we measure and report it).
...
1. **Evaluation harness (KEYSTONE)** — *Phase 0 DONE* (`make bench`: grounding rate D1, coverage D2, externally fed D4 codebook-quality rubric outcome scoring, pre-evaluation D4 protocol validation via `make validate-d4-codebook-quality-protocol`, D4 protocol/result preflight via `make d4-codebook-quality-preflight`, D4 score-time preflight guard via `make bench D4_PROTOCOL=...`, gold-dependent exact code/source-anchor D3 application-validity scoring with raw or versioned validated D3 gold files plus surfaced package provenance, local F1 bootstrap intervals, local same-code/doc span-IoU diagnostics, local Modified Hausdorff diagnostics, exact-key binary system-vs-gold κ/AC1 metadata, human-ceiling exact-metric comparison, human-human κ/α/AC1 metadata when package metrics are present, optional D3 baseline comparisons with local paired delta bootstrap intervals plus per-baseline same-code/doc span-overlap diagnostics, pre-run D3 comparison protocol validation via `make validate-d3-comparison-protocol`, D3 comparison preflight via `make d3-comparison-preflight`, D3 comparison score-time guard via `make bench D3_PROTOCOL=...`, and D3 metric-criteria reporting when configured, externally fed D6 counterfactual identity-swap outcome scoring with code-change Wilson intervals and local mean-Jaccard bootstrap intervals, externally fed D6 stratified correctness/error scoring with accuracy/error-rate Wilson intervals, per-attribute/per-group summaries, per-surface summaries, and max group error-rate gaps, pre-run D6 bias protocol validation via `make validate-d6-bias-protocol`, D6 protocol/result preflight via `make d6-bias-preflight`, D6 score-time preflight guard via `make bench D6_PROTOCOL=...`, reliability/stability D5 with Gwet's AC1, prevalence tables, and local row-bootstrap intervals surfaced when IRR results exist, gold-dependent exact-anchor D7 disconfirmation scoring with recall/precision Wilson intervals, local F1 bootstrap intervals, local same-target/doc span-IoU diagnostics, local Modified Hausdorff diagnostics, exact-key binary system-vs-gold κ/AC1 metadata, optional baseline comparisons with local paired delta bootstrap intervals plus per-baseline span-overlap diagnostics, human-ceiling exact-metric comparison, human-human κ/α/AC1 metadata when package metrics are present, and surfaced package provenance, D3/D7 gold-package validation through Make and top-level `qc_cli.py validate-d3-gold` / `qc_cli.py validate-d7-gold`, `make run-d7-retrieval` export of retrieval-mode candidates for that D7 baseline path, externally fed INV-7 prompt-injection fixture outcome scoring with per-surface/per-attack-type summaries, externally fed D8 GT-fidelity rubric outcome scoring, pre-evaluation D8 GT-fidelity protocol validation via `make validate-d8-gt-fidelity-protocol`, D8 protocol/result preflight via `make d8-gt-fidelity-preflight`, D8 score-time preflight guard via `make bench D8_PROTOCOL=...`, externally fed D9 forced-choice interpretive-preference outcome scoring, pre-evaluation D9 interpretive-preference protocol validation via `make validate-d9-interpretive-preference-protocol`, D9 protocol/result preflight via `make d9-interpretive-preference-preflight`, D9 score-time preflight guard via `make bench D9_PROTOCOL=...`, externally fed confidence-calibration scoring, pre-evaluation confidence-calibration protocol validation via `make validate-confidence-calibration-protocol`, confidence-calibration protocol/result preflight via `make confidence-calibration-preflight`, confidence-calibration score-time preflight guard via `make bench CONFIDENCE_PROTOCOL=...`, D10 cost/latency from real `llm_calls` rows plus optional matching `tool_calls` rows, D10 last-local-run wall-clock metadata, D10 timing artifact packaging with runtime environment metadata, Phase 0 input/configuration hashes with prompt hashes marked `not_run`, Phase 0 artifact verification via `make verify-phase0-benchmark-artifact`, pre-label adjudication protocol validation via `make validate-adjudication-protocol`, protocol/sample package preflight via `make adjudication-protocol-preflight`, unlabeled adjudication sample export via `make adjudication-sample`, completed-response shape validation via `make validate-adjudication-responses`, completed-response protocol/sample preflight via `make adjudication-response-preflight`, D3/D7 gold package import from valid completed responses with optional import-time preflight guard inputs via `make import-adjudication-responses`, and strict Phase 0 manifest assembly for imported D3/D7 package inputs via `make write-phase0-adjudication-package`; deterministic except opt-in embedding export cost and separately generated live INV-7 canary files; `qc_clean/core/bench.py`, `qc_clean/core/d3_comparison_protocol.py`, `qc_clean/core/d3_comparison_preflight.py`, `qc_clean/core/d4_codebook_quality_protocol.py`, `qc_clean/core/d4_codebook_quality_preflight.py`, `qc_clean/core/d6_bias_protocol.py`, `qc_clean/core/d6_bias_preflight.py`, `qc_clean/core/d8_gt_fidelity_protocol.py`, `qc_clean/core/d8_gt_fidelity_preflight.py`, `qc_clean/core/d9_interpretive_preference_protocol.py`, `qc_clean/core/d9_interpretive_preference_preflight.py`, `qc_clean/core/confidence_calibration_protocol.py`, `qc_clean/core/confidence_calibration_preflight.py`, `qc_clean/core/adjudication_protocol.py`, `qc_clean/core/adjudication_preflight.py`, `qc_clean/core/adjudication_response_preflight.py`, `qc_clean/core/adjudication_sample.py`, `qc_clean/core/adjudication_import.py`). Remaining: gold-standard corpora, blind expert ratings, populated D4 LLM-judge/blind-expert evaluations, full D3 Krippendorff's α/semantic agreement and populated human-ceiling comparison data, populated prompt_eval-backed bias stratification + counterfactual identity-cue tests, populated D8 expert GT-fidelity rubric outcomes under a pre-registered protocol, populated blind expert D9 preference outcomes under a pre-registered non-inferiority margin, held-out D7 runs with live baselines, larger independently curated/model-provider adversarial prompt-injection runs, public benchmark timing evidence with controlled environment/baseline context, populated confidence-calibration benchmark — built on `prompt_eval`. *Narrows INV-3 infrastructure but does not close INV-3 until expert labels are collected and scored*, operationalizes INV-5.
   ...
   The adjudication response import script is also available through
   `qc_cli.py import-adjudication-responses ...`; this is CLI parity for
   guarded D3/D7 package-input creation only, not populated adjudication
   evidence.
   ...
   plus protocol/gold/baseline package preflight through
   `make d3-comparison-preflight` and score-time guard enforcement through
   `make bench D3_PROTOCOL=...` with metric-criteria pass/fail/missing reports
   when configured, but actual/live baseline runs on held-out data remain
...
4. **First-class claim ledger** — *MOSTLY DONE object layer* (`ProjectState.claims`, deterministic stage builders, no-claims events, negative-case target refs, CSV/Markdown/CLI/API/MCP surfaces with scope and bounded anchor-detail read rows where applicable, explicit API/MCP/CLI `limit`/`offset` pagination/count metadata or display, API/MCP/browser claim-review first slices with bounded anchor-detail review rows, API/MCP/browser negative-case review first slices, API/MCP/browser/manager relationship-review first slices, API/manager abductive-candidate review first slice, QC-side schema_version=1 process-tracing handoff package with claims/patterns/candidates/anchors/scope/hashes/caveats, code-scoped higher-order claims inheriting exact code-application anchors where available, and relationship evidence strings resolving to exact anchors when uniquely grounded). Process-tracing consumer review on 2026-06-25 accepted the handoff as a QC-side fixture/adaptor input with no requested QC schema changes, while preserving the caveat that runnable PT analysis still needs a PT/workbench research-design/source-packet overlay outside QC. Remaining for full INV-9/10: semantic/source anchoring for arbitrary higher-order prose beyond code-scoped/evidence-string anchor propagation, browser-native abductive candidate review, and deciding whether future LLM schemas emit claim-native outputs.
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICES DONE*: negative-case prompts include bounded claim targets by ID, BM25-style lexical/query-expanded retrieval-first source candidates, opt-in embedding-hybrid retrieval, exact `candidate_id` anchors, configurable disconfirmation model routing, adversarial evidence-bound prompt stance, coverage summaries report challenged/unchallenged targets, API/MCP/browser negative-case review rows expose bounded contrary-anchor details, API/MCP review-list JSON surfaces for claims, negative cases, and relationships support `limit`/`offset` traversal, `make bench` can score D7 against supplied adjudicated gold anchors with recall/precision Wilson intervals, local F1 bootstrap intervals, local same-target/doc span-IoU and Modified Hausdorff diagnostics, and optional baseline comparisons with local paired delta bootstrap intervals plus per-baseline span-overlap diagnostics, `make run-d7-retrieval` exports retrieval-mode candidates with project/corpus/state/trace/budget provenance for that D7 baseline path, `make validate-d7-baseline-package` validates versioned retrieval/live-baseline prediction packages and recognized versioned `BASELINES=` files validate before scoring, `make validate-d7-comparison-protocol` and `make d7-comparison-preflight` validate/cross-check D7 retrieval comparison protocols, versioned gold, predictions, and optional structured metric criteria before scoring, `make compare-d7-retrieval PROTOCOL=... ARTIFACT_DIR=...` can enforce that preflight guard before emitting exact-span local comparison reports for exported packages plus criteria pass/fail/missing status when configured, `_meta.input_hashes` / `_meta.command` report provenance, and optional `report.json` / `manifest.json` artifact packages, `make verify-d7-comparison-artifact` can verify local D7 comparison artifact manifest/report consistency, `make write-d7-comparison-package` can write a strict comparison manifest after validation/preflight, `make compare-d7-package` can run the same comparison from that manifest, `ReviewManager` plus API/MCP/browser first slices can adjudicate claim objects including negative-case claim rows, `ReviewManager` plus API/MCP/browser first slices can adjudicate code/entity relationships, `make validate-adjudication-protocol` can validate pre-label protocol metadata, `make adjudication-protocol-preflight` can cross-check protocol/sample package matching before labeling, `make adjudication-sample` can export unlabeled sample packets for human/expert review inputs, `make validate-adjudication-responses` can validate completed response package shape/completeness, `make adjudication-response-preflight` can check completed responses against protocol/sample provenance before import, `make import-adjudication-responses ... PREFLIGHT_PROTOCOL=protocol.json PREFLIGHT_SAMPLE=sample.json` can convert valid completed code-application/negative-case responses into D3/D7 gold package inputs with an optional import-time preflight guard, and `make write-phase0-adjudication-package` can assemble a strict Phase 0 manifest from those versioned package inputs. Remaining for full INV-2/INV-3/INV-6/INV-10: validated embedding/adversarial retrieval policy, held-out D7 gold-set evaluation with live baselines, held-out retrieval-mode comparisons, and actual expert adjudication runs.
6. **Instruction/data separation + prompt-injection tests** — *FIRST SLICES DONE*: first-party raw transcript/segment prompt data and downstream LLM/codebook artifacts are line-prefixed as untrusted, current prompt override surfaces require bare protected data placeholders, reject undeclared metadata placeholders, require renderer values to be declared as required data, optional data, or metadata, reject raw/unwrapped values supplied for declared data placeholders, reject structured or multi-line values supplied for declared metadata placeholders, bookend rendered custom overrides with a repo-owned data-boundary wrapper/reminder, and are checked against a central source/registry lint, deterministic prompt-capture regressions exist, `make bench` can score externally supplied prompt-injection fixture outcomes, `make run-inv7-fixtures` and `make run-inv7-live-fixtures` write schema_version=1 INV-7 packages whose built-in fixture sets include current custom prompt override surfaces, `make run-inv7-live-fixtures FIXTURES=manifest.json` can execute external schema_version=1 live fixture manifests including held-out manifests with prompt-frozen/contamination-checked/pre-run-registration invariants, live fixture runner output carries exact prompt SHA-256 hashes, `make validate-inv7-live-protocol` validates pre-run protocol metadata, `make inv7-live-preflight` checks live result packages against registered protocol metadata and prompt hashes before scoring, `make bench INV7_PROTOCOL=... PROMPT_INJECTION=...` can enforce that preflight guard before scorecard/output/artifact writes, `make validate-inv7-package` validates result package metadata and optional prompt-hash consistency, `make compare-inv7-packages` / `qc_cli.py compare-inv7-packages` compare already-produced package outcomes, `docs/benchmarks/inv7_live_canary_v2_2026_06_22/` records a committed/scored fixture-set v2 live canary artifact, `docs/benchmarks/inv7_external_heldout_smoke_2026_06_23/` records a committed/scored 3-fixture external held-out smoke artifact, and `docs/benchmarks/inv7_heldout_live_v1_2026_06_23/` records a committed/scored 10-fixture external held-out live v1 artifact. Remaining for full INV-7: larger and independently curated adversarial suites plus model/provider comparisons before any robustness claim; package comparison reports compare observed outcomes only and do not prove robustness or model/provider superiority.
7. **True theoretical sampling + per-category saturation** — *PARTIAL*: category adequacy diagnostics are surfaced separately from codebook stability, existing document suggestions consume those diagnostic gaps, loaded-document candidate packages can be exported from those suggestions using explicit repo-local project stores when needed, selected-candidate result packages can be recorded, pre-run theoretical-sampling protocol packages can be validated, concrete candidate/result packages can be preflighted against those protocols, top-level `qc_cli.py` wrappers expose those same protocol/preflight/export surfaces, and `docs/benchmarks/theoretical_sampling_smoke_2026_06_23/` commits a synthetic repo-local workflow/provenance smoke artifact with a passing preflight report. Remaining work is collecting/selecting new data beyond already-loaded uncoded docs, populated sampling-result evidence from real sampling cycles, and expert/held-out GT-fidelity evaluation before any true saturation claim (only if the GT path claims theory generation).
8. **Corpus-boundary / scope-condition contract** — *FIRST SLICES DONE*: optional `ProjectState.corpus_scope` records phenomenon, population, sampling frame, inclusion/exclusion criteria, and caveats; CLI/API read/update surfaces are available; CLI/MCP project creation can set scope at creation time; Markdown export surfaces scope before analytic claims, CSV/Markdown claim rows carry compact claim-scope and corpus-boundary context, Markdown/JSON/CSV exports warn when claim-bearing exports have no recorded scope, an empty scope record, or a population without a sampling frame, `make lint-scope-phrasing` scans arbitrary text for risky population-generalizing phrases under missing/under-specified scope, `make export-publish-preflight ... SCOPE_LINT=1`, `qc_cli.py project export ... --publish-preflight --scope-lint`, and MCP `qc_export_markdown(..., publish_preflight=True, scope_lint=True)` can block risky Markdown/text handoff artifacts before publish/handoff, `make bench` reports deterministic scope-record adequacy status/field completeness, and `make validate-sampling-frame-adequacy-protocol` / `make sampling-frame-adequacy-preflight` plus `qc_cli.py` wrappers validate/preflight future sampling-frame reviewer result packages. Remaining: populated reviewer evidence and any score-time guard before sampling-frame adequacy can be treated as more than a protocol/preflight substrate.
   `qc_cli.py lint-scope-phrasing` is the top-level CLI alias for the same scope-language lint, and `qc_cli.py export-publish-preflight --scope-lint` / `qc_cli.py project export --publish-preflight --scope-lint` / MCP `publish_preflight=True, scope_lint=True` expose the opt-in publish-preflight gate; the sampling-frame adequacy protocol/preflight surfaces are not populated reviewer evidence, not sampling-frame adequacy evidence, not population-generalization permission, not methodological-validity evidence, and not SOTA evidence.
9. **Stale-output handling** — *INV-11 explicit add-docs recode hook + hard-invalidation default + opt-in thematic and GT higher-order refresh + project-level refresh policy slices DONE*; remaining: decide whether auto-refresh becomes the universal/new-project default after live validation and budget-policy review.
10. **Features:** multi-model consensus; active learning from review; collaborative coding; retrieval grounding.
11. **Tamper-evident audit substrate** — *FIRST HASH-MANIFEST + LOCAL VERIFY + PREFLIGHT + EVENT-LOG + SQLITE-MIRROR SLICES DONE*: `make export-audit-manifest` / `qc_cli.py export-audit-manifest` write schema_version=1 hash manifests for existing JSON/CSV/Markdown/QDPX export artifacts with project-state and per-file SHA-256 hashes; `make verify-export-audit-manifest` / `qc_cli.py verify-export-audit-manifest` verify manifest self-hash, artifact sizes/hashes, and optional current project-state hash; `make export-publish-preflight` / `qc_cli.py export-publish-preflight` are strict local publish/handoff gates that require an existing verified export-audit manifest and can include the current project-state hash when `ID=<project_id>` / `--project-id` is supplied, with optional `SCOPE_LINT=1` / `--scope-lint` report-discipline checks for Markdown/text artifacts; `qc_cli.py project export --audit-manifest ... --publish-preflight [--scope-lint]` and MCP `qc_export_markdown/json(..., audit_manifest=True, publish_preflight=True)` can run that same gate inline after writing the export manifest; local audit scripts and `qc_cli.py project export --audit-manifest ... --audit-log ... [--audit-db ...]` can append hash-linked JSONL events and optionally mirror them into SQLite, MCP JSON/Markdown export tools can append confined event-log sidecars with `audit_event_log=True` and confined SQLite mirrors with `audit_event_db=True`, `make verify-export-audit-log` / `qc_cli.py verify-export-audit-log` verify event self-hashes and previous-event links, and `make mirror-export-audit-db` / `qc_cli.py mirror-export-audit-db` plus `make verify-export-audit-db` / `qc_cli.py verify-export-audit-db` can mirror verified JSONL events into local SQLite and verify stored payload hashes/order. Remaining: signed/external or append-only storage semantics and default export/event-log policy decisions. Do not call the current manifest/verification/preflight/event-log/SQLite-mirror set a full tamper-evident audit log.

...
1. **Evaluation harness (KEYSTONE)** — *Phase 0 DONE* (`make bench`: grounding rate D1, coverage D2, reliability/stability D5, gold-dependent exact-anchor D7 disconfirmation scoring with recall/precision Wilson intervals, and externally fed INV-7 prompt-injection fixture outcome scoring; deterministic; `qc_clean/core/bench.py`). Remaining: gold-standard corpora, blind expert ratings, agreement-vs-gold (κ/α/AC1, IoU/Hausdorff), bias stratification + counterfactual identity-cue tests, held-out D7 runs with baselines and F1/bootstrap intervals, live adversarial prompt-injection runs, confidence calibration, baselines — built on `prompt_eval`. *Closes INV-3*, operationalizes INV-5.

...
- **Inter-rater reliability** (`project irr`) — runs coding N times with prompt variation (and optionally across multiple models) and computes **percent agreement**, **Cohen's kappa** (2 passes), and **Fleiss' kappa** (2+), interpreted on the **Landis & Koch (1977)** scale. Two units of analysis:
  ...
  - *`--application-level` (opt-in):* runs **exhaustive** passes (per-segment decisions; INV-8) and builds a **segment × code** matrix from the segment-anchored applications — answers "when at least one pass applied code C to segment S, did the other passes also apply C to S?" This is a stronger code-application consistency number than codebook discovery, reported alongside the discovery metrics (`IRRResult.application_*`). Current denominator caveat: shared `no_code` / null decisions are not counted as agreement rows yet. Scope: thematic exhaustive; GT application-level is a follow-up. Still subject to the pseudo-replication caveat (§15, INV-3): LLM passes are not independent human raters.
...
| "LLM-pass segment × code application agreement with `--application-level` (positive code-application cells; no-code agreement not counted yet)" | "Human inter-rater reliability" / "methodological validity" / "all segment decisions agree" |
...
3. **Segment universe** — *DONE* (`segmentation.py` registry + `ProjectState.segments` + `compute_coverage`; **exhaustive per-segment coding** via `project run --exhaustive` gives examined-and-judged coverage and segment-anchored applications — INV-8 met in that mode; `project irr --application-level` now reports positive segment × code application agreement). Remaining: decide whether exhaustive becomes the default after live validation, and whether to add explicit no-code/null segment agreement to the IRR report.

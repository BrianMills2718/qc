...
11. **Tamper-evident audit substrate** — *FIRST HASH-MANIFEST + LOCAL VERIFY + PREFLIGHT + EVENT-LOG + SQLITE-MIRROR SLICES DONE*: `make export-audit-manifest` writes schema_version=1 hash manifests for existing JSON/CSV/Markdown/QDPX export artifacts with project-state and per-file SHA-256 hashes; `make verify-export-audit-manifest` verifies manifest self-hash, artifact sizes/hashes, and optional current project-state hash; `make export-publish-preflight` is a strict local publish/handoff gate that requires an existing verified export-audit manifest and can include the current project-state hash when `ID=<project_id>` is supplied; local audit scripts and `qc_cli.py project export --audit-manifest ... --audit-log ... [--audit-db ...]` can append hash-linked JSONL events and optionally mirror them into SQLite, MCP JSON/Markdown export tools can append confined event-log sidecars with `audit_event_log=True` and confined SQLite mirrors with `audit_event_db=True`, `make verify-export-audit-log` verifies event self-hashes and previous-event links, and `make mirror-export-audit-db` / `make verify-export-audit-db` can mirror verified JSONL events into local SQLite and verify stored payload hashes/order. Remaining: signed/external or append-only storage semantics and default export/event-log policy decisions. Do not call the current manifest/verification/preflight/event-log/SQLite-mirror set a full tamper-evident audit log.

...
**Measured (quantified, but not validity):** **grounding rate** (D1) and **coverage** (D2 — segments with anchored evidence / total) via `verify_grounding`/`compute_coverage`/`make bench`; **LLM-pass agreement** (`project irr` — codebook-discovery by default, **positive segment × code application agreement plus segment-decision agreement** with `--application-level`) and **stability** (`project stability`) — computational consistency, not human inter-rater reliability and not correctness. `make bench` also has a **gold-dependent D3 application-validity scorecard substrate** (exact code/source-anchor TP/FP/FN, recall, precision, F1, recall/precision Wilson intervals, local exact-key F1 bootstrap interval, local same-code/doc char-span IoU overlap diagnostic, and local discrete Modified Hausdorff distance diagnostic) when adjudicated application gold is supplied in project metadata or via external `D3_GOLD=d3_gold.json`, including versioned D3 gold-set packages validated by `make validate-d3-gold` with package provenance surfaced in the scorecard, a **gold-dependent D7 disconfirmation scorecard substrate** (exact target-claim/source-anchor TP/FP/FN, recall, precision, F1, recall/precision Wilson intervals, local exact-key F1 bootstrap interval, optional baseline comparisons with local paired exact-key delta bootstrap intervals, and versioned-package provenance when supplied) when adjudicated contrary-evidence gold anchors are supplied in project metadata or via an external `GOLD=gold.json` file, an **INV-7 prompt-injection scorecard substrate** when fixture outcomes are supplied in project metadata or via `PROMPT_INJECTION=inv7.json`, with deterministic structural and opt-in live canary fixture runners able to produce compatible input files, a **D10 cost/latency scorecard substrate** from real `llm_client` observability rows when matching traces exist plus last-local-run wall-clock metadata from `project run`, `_meta.input_hashes` provenance for loaded project/corpus/external benchmark files, and a **diagnostic-only INV-4 category adequacy section** that reports category properties/dimensions/support gaps; no held-out D3/D7 benchmark, D3 human-ceiling agreement, live baseline run on held-out data, committed/scored live adversarial prompt-injection benchmark result, versioned public benchmark timing artifact, or true GT saturation assessment has been run.

...
**Incremental** (`incremental_coding.py`) — `project recode`: codes only *uncoded* documents against the existing codebook, merges, then re-runs downstream state-driven stages (Cross-Interview → Negative Case). Deliberately omits stages that depend on ctx fields from a coding stage that is not re-run (Perspective/Relationship/Synthesis, Axial/Selective/Theory). **Staleness hazard (INV-11):** because those stages do not re-run, after a recode the state can hold *fresh* `code_applications` alongside *stale* `synthesis`, `perspective_analysis`, `entities`, `core_categories`, and `theoretical_model` that reflect the old corpus — with no warning. New documents can change all of those. Until recode invalidates or flags them, treat post-recode higher-order outputs as possibly stale.
...
- **INV-8 — Explicit segment universe (a defensible denominator).** The architecture must distinguish "not coded," "not examined," and "examined but judged irrelevant" — i.e., a stable registry of textual units (segment IDs) against which every coding decision (including null decisions) is recorded. *Status: UNMET.* Without it, coverage, application-level agreement, consensus, prevalence, co-occurrence, and "absence of evidence" all lack a defined denominator (this is the root cause of the cross-interview and IRR caveats in §7/§11). The thematic path's example-quote applications are *not* such a universe.
...
- **INV-11 — No stale higher-order outputs.** When the corpus changes (e.g. `project recode`), any downstream output not recomputed must be invalidated or flagged, never silently retained. *Status: UNMET* (incremental recode leaves stale `synthesis`/`perspective`/`entities`/GT outputs; §7). This is fail-loud applied at the inferential level.
...
6. **Stale-output invalidation** — `project recode` (and any corpus mutation) must invalidate/flag non-recomputed downstream outputs. *Closes INV-11* (a cheap, near-term fix).

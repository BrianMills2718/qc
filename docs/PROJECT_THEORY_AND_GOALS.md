...
11. **Tamper-evident audit substrate** — *FIRST HASH-MANIFEST SLICE DONE*: `make export-audit-manifest` writes schema_version=1 hash manifests for existing JSON/CSV/Markdown/QDPX export artifacts with project-state and per-file SHA-256 hashes. Remaining: append-only/signed log, automatic export policy integration, and optional DB-backed audit events. Do not call the current manifest a full tamper-evident audit log.

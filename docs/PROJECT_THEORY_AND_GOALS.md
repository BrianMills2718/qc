...
- **`observed_patterns: List[ObservedPattern]`** — first-class descriptive pattern records produced from deterministic stage outputs. Current producers: Cross-Interview consensus, divergent-code, and code-co-occurrence records. These are descriptive pattern accounting objects with `causal_interpretation_status="descriptive_only"` by default; they are not causal proof or abductive explanations.

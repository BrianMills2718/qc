...
1. **Evaluation harness (KEYSTONE)** — *Phase 0 DONE* (`make bench`: grounding rate D1, coverage D2, gold-dependent exact code/source-anchor D3 application-validity scoring with raw or versioned validated D3 gold files plus surfaced package provenance and local F1 bootstrap intervals, reliability/stability D5, gold-dependent exact-anchor D7 disconfirmation scoring with recall/precision Wilson intervals, local F1 bootstrap intervals, optional baseline point comparisons, and surfaced package provenance, `make run-d7-retrieval` export of retrieval-mode candidates for that D7 baseline path, externally fed INV-7 prompt-injection fixture outcome scoring, D10 cost/latency from real observability rows, D10 last-local-run wall-clock metadata, and Phase 0 input hashes; deterministic except opt-in embedding export cost; `qc_clean/core/bench.py`). Remaining: gold-standard corpora, blind expert ratings, richer agreement-vs-gold (κ/α/AC1, IoU/Hausdorff), bias stratification + counterfactual identity-cue tests, held-out D7 runs with live baselines and interval-tested baseline deltas, live adversarial prompt-injection runs, versioned benchmark timing artifacts, confidence calibration — built on `prompt_eval`. *Closes INV-3*, operationalizes INV-5.
...
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICES DONE*: negative-case prompts include bounded claim targets by ID, BM25-style lexical/query-expanded retrieval-first source candidates, opt-in embedding-hybrid retrieval, exact `candidate_id` anchors, configurable disconfirmation model routing, adversarial evidence-bound prompt stance, coverage summaries report challenged/unchallenged targets, `make bench` can score D7 against supplied adjudicated gold anchors with recall/precision Wilson intervals, local F1 bootstrap intervals, and optional baseline point comparisons, `make run-d7-retrieval` exports retrieval-mode candidates for that D7 baseline path, `make compare-d7-retrieval` emits exact-span point-estimate comparison reports for exported packages, and `ReviewManager` can adjudicate claim objects. Remaining for full INV-2/INV-6/INV-10: validated embedding/adversarial retrieval policy, held-out D7 gold-set evaluation with live baselines, retrieval-mode comparisons, interval-tested baseline deltas, browser-native review, relationship/negative-case review UX, and expert adjudication protocol.

...
4. **First-class claim ledger** — *MOSTLY DONE object layer* (`ProjectState.claims`, deterministic stage builders, no-claims events, negative-case target refs, CSV/Markdown/CLI/API/MCP surfaces, API claim-review listing). Remaining for full INV-9/10: stronger source anchoring for higher-order claims, browser-native claim review UI, and deciding whether future LLM schemas emit claim-native outputs.

...
4. **First-class claim ledger** — *MOSTLY DONE object layer* (`ProjectState.claims`, deterministic stage builders, no-claims events, negative-case target refs, CSV/Markdown/CLI/API/MCP surfaces, API/MCP/browser claim-review first slices, API/MCP/browser negative-case review first slices, API/MCP/browser/manager relationship-review first slices, and code-scoped higher-order claims inheriting exact code-application anchors where available). Remaining for full INV-9/10: semantic/source anchoring for arbitrary higher-order prose beyond code-scoped anchor propagation, and deciding whether future LLM schemas emit claim-native outputs.

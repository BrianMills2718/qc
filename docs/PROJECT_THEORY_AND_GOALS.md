...
1. **Evaluation harness (KEYSTONE)** — *Phase 0 DONE* (`make bench`: grounding rate D1, coverage D2, gold-dependent exact code/source-anchor D3 application-validity scoring with raw or versioned validated D3 gold files plus surfaced package provenance, local F1 bootstrap intervals, and local same-code/doc span-IoU diagnostics, reliability/stability D5, gold-dependent exact-anchor D7 disconfirmation scoring with recall/precision Wilson intervals, local F1 bootstrap intervals, optional baseline comparisons with local paired delta bootstrap intervals, and surfaced package provenance, `make run-d7-retrieval` export of retrieval-mode candidates for that D7 baseline path, externally fed INV-7 prompt-injection fixture outcome scoring, D10 cost/latency from real observability rows, D10 last-local-run wall-clock metadata, and Phase 0 input hashes; deterministic except opt-in embedding export cost; `qc_clean/core/bench.py`). Remaining: gold-standard corpora, blind expert ratings, richer agreement-vs-gold (κ/α/AC1, Modified Hausdorff), bias stratification + counterfactual identity-cue tests, held-out D7 runs with live baselines, live adversarial prompt-injection runs, versioned benchmark timing artifacts, confidence calibration — built on `prompt_eval`. *Closes INV-3*, operationalizes INV-5.

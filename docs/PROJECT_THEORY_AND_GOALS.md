...
intervals via `verify_grounding`/`compute_coverage`/`make bench`;
...
scorecard substrate** from real `llm_client` observability rows when matching
traces exist plus last-local-run wall-clock metadata from `project run`,
`_meta.input_hashes` provenance for loaded project/corpus/external benchmark
files, and a **diagnostic-only INV-4 category adequacy section** that reports
...
committed/scored live adversarial prompt-injection benchmark result, versioned
public benchmark timing artifact, full D3 Krippendorff's α/semantic agreement,

...
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICES DONE*: negative-case prompts include bounded claim targets by ID, BM25-style lexical/query-expanded retrieval-first source candidates, opt-in embedding-hybrid retrieval, exact `candidate_id` anchors, configurable disconfirmation model routing, adversarial evidence-bound prompt stance, coverage summaries report challenged/unchallenged targets, `make bench` can score D7 against supplied adjudicated gold anchors with recall/precision Wilson intervals and optional baseline point comparisons, `make run-d7-retrieval` exports retrieval-mode candidates for that D7 baseline path, and `ReviewManager` can adjudicate claim objects. Remaining for full INV-2/INV-6/INV-10: validated embedding/adversarial retrieval policy, held-out D7 gold-set evaluation with live baselines, retrieval-mode comparisons, F1/bootstrap/interval-tested baseline deltas, browser-native review, relationship/negative-case review UX, and expert adjudication protocol.

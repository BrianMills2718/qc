...
- **INV-10 — Adjudication beyond code labels.** Human review must reach applications, claims, relationships, and negative cases — not only code objects. *Status: **PARTIAL**.* `ReviewManager` supports code applications and `target_type="claim"` decisions for approve/reject/modify, updating `AnalyticClaim.adjudication_status` and `revision_history`; `ReviewSummary` includes `claims_count` plus active/inactive review-decision counters; `/projects/{id}/review/claims` and MCP `qc_review_claims` expose bounded claim review rows for API/browser and agent clients; MCP `qc_review_decisions` can apply claim decisions with rationale for agent-drivable review; the browser review page now has a Claims mode that lists claim cards and submits claim approve/reject/modify decisions through the shared decision endpoint. Remaining gaps: relationship/negative-case review UX is not first-class, and there is no sampled expert adjudication protocol yet (INV-3). A code label can be fine while its quote applications or downstream claims are wrong; marking the code human-reviewed must not launder unreviewed LLM inferences below it.
...
4. **First-class claim ledger** — *MOSTLY DONE object layer* (`ProjectState.claims`, deterministic stage builders, no-claims events, negative-case target refs, CSV/Markdown/CLI/API/MCP surfaces, API/MCP/browser claim-review first slices). Remaining for full INV-9/10: stronger source anchoring for higher-order claims, relationship/negative-case review UX, and deciding whether future LLM schemas emit claim-native outputs.
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICES DONE*: negative-case prompts include bounded claim targets by ID, BM25-style lexical/query-expanded retrieval-first source candidates, opt-in embedding-hybrid retrieval, exact `candidate_id` anchors, configurable disconfirmation model routing, adversarial evidence-bound prompt stance, coverage summaries report challenged/unchallenged targets, `make bench` can score D7 against supplied adjudicated gold anchors with recall/precision Wilson intervals, local F1 bootstrap intervals, and optional baseline comparisons with local paired delta bootstrap intervals, `make run-d7-retrieval` exports retrieval-mode candidates for that D7 baseline path, `make compare-d7-retrieval` emits exact-span local comparison reports for exported packages, and `ReviewManager` plus API/MCP/browser first slices can adjudicate claim objects. Remaining for full INV-2/INV-6/INV-10: validated embedding/adversarial retrieval policy, held-out D7 gold-set evaluation with live baselines, held-out retrieval-mode comparisons, relationship/negative-case review UX, and expert adjudication protocol.

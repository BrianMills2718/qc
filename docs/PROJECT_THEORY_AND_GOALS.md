...
**Incremental** (`incremental_coding.py`) — `project recode`: codes only *uncoded* documents against the existing codebook, merges, then re-runs downstream state-driven stages (Cross-Interview → Negative Case). `project add-docs --recode` explicitly adds documents and invokes that same recode path in one command; plain `add-docs` does not silently spend model budget. Default recode deliberately omits stages that depend on ctx fields from a coding stage that is not re-run (Perspective/Relationship/Synthesis, Axial/Selective/Theory). **Staleness handling (INV-11):** default recode clears populated stale outputs (`synthesis`, `perspective_analysis`, `code_relationships`, `entities`, `entity_relationships`, `core_categories`, `theoretical_model`), removes stale phase results and stale claim-ledger rows from the invalidated source stages, and appends a `data_warnings` entry naming what was invalidated. `project recode --refresh-higher-order` and `project add-docs --recode --refresh-higher-order` can opt into methodology-specific refresh: default/thematic projects reconstruct Phase 1 JSON from the current codebook and rerun Perspective → Relationship → Synthesis; grounded-theory projects reconstruct GT open-code context from the current codebook and rerun Axial → Selective → Theory Integration; both then rerun Cross-Interview → Negative Case. INV-11 remains partial because refresh is opt-in and not a full default auto-recompute policy.
...
- **INV-11 — No stale higher-order outputs.** When the corpus changes (e.g. `project recode`), any downstream output not recomputed must be invalidated or flagged, never silently retained. *Status: **PARTIAL (explicit add-docs recode hook + hard-invalidation default + opt-in thematic and GT higher-order refresh slices).*** `project add-docs --recode` can now perform corpus mutation and invoke incremental recode in one opt-in command; plain `add-docs` still does not silently spend model budget. Default `IncrementalCodingStage` clears stale higher-order outputs it does not recompute (`synthesis`, `perspective_analysis`, `code_relationships`, `entities`, `entity_relationships`, `core_categories`, `theoretical_model`), removes matching stale phase results and claim-ledger rows for invalidated source stages, marks review decisions targeting removed claims inactive while retaining them as historical audit records, and appends a `data_warnings` entry naming what was invalidated. That warning is surfaced on the human + agent surfaces that matter: CLI human formatter, API JSON, JSON export, **Markdown export** (prominent block), MCP `qc_recode`/`qc_get_synthesis`/`qc_get_codebook`, and graph code/entity data responses. For default/thematic projects, `--refresh-higher-order` opt-in rebuilds Phase 1 JSON from the updated codebook and reruns Perspective, Relationship, and Synthesis before Cross-Interview and Negative Case. For grounded-theory projects, it rebuilds GT open-code context from the updated codebook and reruns Axial, Selective, and Theory Integration before Cross-Interview and Negative Case. *Remaining gap (why PARTIAL, not MET):* refresh is opt-in, default recode still uses hard invalidation, and no full automatic corpus-mutation policy exists. Locked by `tests/test_incremental_staleness_inv11.py`, `tests/test_incremental.py`, and `tests/test_project_commands.py`.
...
9. **Stale-output handling** — *INV-11 explicit add-docs recode hook + hard-invalidation default + opt-in thematic and GT higher-order refresh slices DONE*; remaining: default auto-refresh policy on corpus mutation.

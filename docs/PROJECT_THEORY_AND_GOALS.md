...
4. **First-class claim ledger** — *MOSTLY DONE object layer* (`ProjectState.claims`, deterministic stage builders, no-claims events, negative-case target refs, CSV/Markdown/CLI/API/MCP surfaces with scope and bounded anchor-detail read rows where applicable, explicit API/MCP/CLI `limit`/`offset` pagination/count metadata or display, API/MCP/browser claim-review first slices with bounded anchor-detail review rows, API/MCP/browser negative-case review first slices, API/MCP/browser/manager relationship-review first slices, code-scoped higher-order claims inheriting exact code-application anchors where available, and relationship evidence strings resolving to exact anchors when uniquely grounded). Remaining for full INV-9/10: semantic/source anchoring for arbitrary higher-order prose beyond code-scoped/evidence-string anchor propagation, and deciding whether future LLM schemas emit claim-native outputs.

...
Two pipelines, each a fixed ordered sequence of stages over `ProjectState`. **Disconfirmation runs last in both** and now receives bounded claim-ledger targets by claim ID, but it remains same-model, prompt-memory disconfirmation rather than retrieval-first adversarial evidence search (INV-2/6 are still incomplete; see §13.1).
...
- **Negative Case** (`negative_case.py`) — LLM searches for disconfirming evidence against the codebook, cross-interview claims, and a bounded `ProjectState.claims` target list. *Schema:* `NegativeCaseResponse` (negative_cases + optional `target_claim_id`, overall_assessment + analytical_memo). *Writes:* `negative_case` + `methodological` memos and negative-case claim objects with contrary anchors/target claim IDs where resolvable. *Runs:* when codebook has codes. **Status: experimental** (INV-2 — same-model/prompt lineage; can launder confirmation).
...
- **INV-2 — Source-anchored, retrieval-first disconfirmation.** Negative cases must be drawn by retrieving candidate contrary passages from the corpus *first*, then interpreting — ideally with a different model/adversarial prompt — each anchored (INV-1) and human-adjudicated **before the claim set is treated as final**. The current pass is claim-ledger-routed, so synthesis/perspective/entity/GT claims can be listed as targets, but it is still a same-model, prompt-memory pass rather than retrieval-first contrary-evidence search. *Status: UNMET* (same-model, memory-first pass that can launder confirmation). Disconfirmation is experimental and **not** credibility evidence.
...
- **INV-6 — Disconfirmation covers the *final* claim set.** Every output-producing stage's claims must be subject to disconfirmation — including the last one. *Status: **PARTIAL (ledger-routed first slice).*** `NegativeCaseStage` now runs **last** and ingests the codebook, the cross-interview `cross_case` memo, and a bounded `ProjectState.claims` target list by claim ID. `NegativeCase.target_claim_id` lets found contrary evidence attach to exact challenged claims, and `summarize_disconfirmation_coverage()` reports challenged vs. unchallenged claim targets on CLI/API/MCP surfaces. **Remaining gaps:** target lists are bounded/truncated for prompt size; the pass is still same-model, memory-first, not retrieval-first/adversarial (INV-2); not every target is guaranteed to receive a negative-case search result; and the negative-case stage's own assessment is not challenged by a later stage. Do **not** say "all final claims survived disconfirmation."
...
| "Experimental ledger-routed disconfirmation over bounded claim targets" | "Negative-case analysis establishes credibility" / "all final claims were disconfirmed" (same-model, bounded prompt pass; INV-2/6) |
...
- **Disconfirmation can launder confirmation** (INV-2). Same model+prompt lineage refuting its own codes/claims may stay inside its own assumptions. The current pass is ledger-routed by claim ID, which improves coverage accounting but is still not retrieval-first evidence search. The real version needs a different model/adversarial prompt, retrieval-first search, anchored spans, and human adjudication. *Absence of a found negative case is not evidence none exists.*
...
LLM bias may be systematic not random (INV-3/5); reliability ≠ validity; pseudo-replication; the denominator problem (INV-8 default path); claim ledger rows can be provisional/unanchored/unadjudicated (INV-9/10); GT fidelity partial ("GT-inspired," not "full GT"); disconfirmation ledger-routed but still experimental and not retrieval-first (INV-2/6); confidence uncalibrated; corpus boundary unstated; incremental staleness now flagged but outputs not auto-refreshed (INV-11); audit substrate not tamper-evident; methodological validity unmeasured; non-determinism inherent (we measure and report it).
...
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICE DONE*: negative-case prompts include bounded claim targets by ID, coverage summaries report challenged/unchallenged targets, and `ReviewManager` can adjudicate claim objects. Remaining for full INV-6/INV-10: retrieval-first/adversarial search over claim targets, browser-native review, relationship/negative-case review UX, and expert adjudication protocol. Hardened retrieval-first, different-model disconfirmation *closes INV-2*.

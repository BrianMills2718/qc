...
- **INV-11 — No stale higher-order outputs.** When the corpus changes (e.g. `project recode`), any downstream output not recomputed must be invalidated or flagged, never silently retained. *Status: MET (flagged, not invalidated).* `IncrementalCodingStage` now appends a `data_warnings` entry naming the populated outputs it did not recompute (`synthesis`/`perspective`/`entities`/`entity_relationships`/`core_categories`/`theoretical_model`); the outputs are retained (non-destructive) but no longer silently presented as current. Locked by `tests/test_incremental_staleness_inv11.py`. *Not yet done:* automatic re-computation or hard invalidation — consumers must honor the warning.

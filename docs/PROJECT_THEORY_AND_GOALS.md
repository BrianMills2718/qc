...
Two pipelines, each a fixed ordered sequence of stages over `ProjectState`. **Disconfirmation runs last in both** and now retrieves bounded BM25-style lexical/query-expanded source-candidate passages for claim-ledger targets before adversarially framed LLM interpretation. It remains non-embedding retrieval and not human-adjudicated or held-out D7-evaluated; interpretation can use `disconfirmation_model_name` but no validated default reviewer has been selected, so INV-2/6 are still incomplete; see §13.1.
...
- **Negative Case** (`negative_case.py`) — retrieves bounded BM25-style lexical/query-expanded source-candidate passages for claim-ledger targets, then asks an adversarially framed LLM reviewer to assess whether those candidates disconfirm the codebook, cross-interview claims, or other live claims. `PipelineContext.disconfirmation_model_name` can route this interpretation through a separate model; unset falls back to `model_name`. *Schema:* `NegativeCaseResponse` (negative_cases + optional `target_claim_id` / `candidate_id`, overall_assessment + analytical_memo). *Writes:* `negative_case` + `methodological` memos and negative-case claim objects with contrary anchors/target claim IDs where resolvable; valid `candidate_id`s attach exact candidate anchors. *Runs:* when codebook has codes. **Status: experimental** (INV-2 partial — deterministic lexical/query-expanded retrieval-first and different-model-capable, not semantic/embedding retrieval and not held-out D7-evaluated).
...
**Measured (quantified, but not validity):** **grounding rate** (D1) and **coverage** (D2 — segments with anchored evidence / total) via `verify_grounding`/`compute_coverage`/`make bench`; **LLM-pass agreement** (`project irr` — codebook-discovery by default, **positive segment × code application agreement plus segment-decision agreement** with `--application-level`) and **stability** (`project stability`) — computational consistency, not human inter-rater reliability and not correctness. `make bench` also has a **gold-dependent D7 disconfirmation scorecard substrate** (exact target-claim/source-anchor TP/FP/FN, recall, precision, F1, recall/precision Wilson intervals, and optional baseline point comparisons) when adjudicated contrary-evidence gold anchors are supplied in project metadata or via an external `GOLD=gold.json` file, an **externally fed INV-7 prompt-injection scorecard substrate** when fixture outcomes are supplied in project metadata or via `PROMPT_INJECTION=inv7.json`, a **D10 cost/latency scorecard substrate** from real `llm_client` observability rows when matching traces exist plus last-local-run wall-clock metadata from `project run`, `_meta.input_hashes` provenance for loaded project/corpus/external benchmark files, and a **diagnostic-only INV-4 category adequacy section** that reports category properties/dimensions/support gaps; no held-out D7 benchmark, live baseline run, live adversarial prompt-injection benchmark, versioned public benchmark timing artifact, or true GT saturation assessment has been run.
...
- **INV-2 — Source-anchored, retrieval-first disconfirmation.** Negative cases must be drawn by retrieving candidate contrary passages from the corpus *first*, then interpreting — ideally with a different model/adversarial prompt — each anchored (INV-1) and human-adjudicated **before the claim set is treated as final**. *Status: **PARTIAL (BM25-style lexical/query-expanded retrieval-first + adversarial routing + D7 scoring substrate slices).*** `qc_clean/core/disconfirmation.py` retrieves bounded char-anchored segment candidates for live claim-ledger targets before LLM interpretation using BM25-style term weighting, configurable contradiction-cue boosts, and deterministic query-expansion terms that can surface contrary passages without exact claim-term overlap; `NegativeCaseStage` sends retrieved candidates instead of the full corpus, frames interpretation as skeptical/adversarial evidence review, and can route through `PipelineContext.disconfirmation_model_name`; `NegativeCase.candidate_id` lets returned cases attach exact candidate anchors, and invalid candidate IDs fail loud; `make bench` computes exact D7 recall/precision/F1 plus 95% Wilson intervals for recall/precision when adjudicated contrary-evidence gold anchors are present in project metadata or supplied through an external gold file, and can score externally supplied baseline predictions against the same gold. **Remaining gaps:** retrieval is still lexical/query-expanded, not semantic/embedding/adversarial retrieval; no validated default adversarial model/provider policy has been chosen; target/candidate counts are bounded; zero retrieved candidates is not evidence of no contrary evidence; candidate results are not human-adjudicated; no held-out D7 gold-set benchmark with live baselines, interval-tested baseline deltas, or F1/bootstrap intervals has run. Disconfirmation is experimental and **not** credibility evidence.
...
| "Experimental BM25-style lexical/query-expanded retrieval-first disconfirmation over bounded claim targets and source candidates, with configurable adversarial reviewer model" | "Negative-case analysis establishes credibility" / "all final claims were disconfirmed" (not semantic/embedding retrieval; bounded candidates; no validated default adversarial model or held-out D7 evidence; INV-2/6 still partial) |
...
1. **Evaluation harness (KEYSTONE)** — *Phase 0 DONE* (`make bench`: grounding rate D1, coverage D2, reliability/stability D5, gold-dependent exact-anchor D7 disconfirmation scoring with recall/precision Wilson intervals and optional baseline point comparisons, externally fed INV-7 prompt-injection fixture outcome scoring, D10 cost/latency from real observability rows, D10 last-local-run wall-clock metadata, and Phase 0 input hashes; deterministic; `qc_clean/core/bench.py`). Remaining: gold-standard corpora, blind expert ratings, agreement-vs-gold (κ/α/AC1, IoU/Hausdorff), bias stratification + counterfactual identity-cue tests, held-out D7 runs with live baselines and F1/bootstrap/interval-tested baseline deltas, live adversarial prompt-injection runs, versioned benchmark timing artifacts, confidence calibration — built on `prompt_eval`. *Closes INV-3*, operationalizes INV-5.
...
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICES DONE*: negative-case prompts include bounded claim targets by ID, BM25-style lexical/query-expanded retrieval-first source candidates, exact `candidate_id` anchors, configurable disconfirmation model routing, adversarial evidence-bound prompt stance, coverage summaries report challenged/unchallenged targets, `make bench` can score D7 against supplied adjudicated gold anchors with recall/precision Wilson intervals and optional baseline point comparisons, and `ReviewManager` can adjudicate claim objects. Remaining for full INV-2/INV-6/INV-10: semantic/embedding/adversarial retrieval, validated model/provider policy, held-out D7 gold-set evaluation with live baselines and F1/bootstrap/interval-tested baseline deltas, browser-native review, relationship/negative-case review UX, and expert adjudication protocol.
6. **Instruction/data separation + prompt-injection tests** — *FIRST SLICES DONE*: first-party raw transcript/segment prompt data and downstream LLM/codebook artifacts are line-prefixed as untrusted, current prompt override surfaces require protected data placeholders, deterministic prompt-capture regressions exist, and `make bench` can score externally supplied prompt-injection fixture outcomes. Remaining for full INV-7: broader custom-prompt threat-model tightening, unsafe metadata handling, and live adversarial injection evaluation.

...
1. **Evaluation harness (KEYSTONE)** — *Phase 0 DONE* (`make bench`: grounding rate D1, coverage D2, reliability/stability D5, gold-dependent exact-anchor D7 disconfirmation scoring with recall/precision Wilson intervals, externally fed INV-7 prompt-injection fixture outcome scoring, and D10 cost/latency from real observability rows; deterministic; `qc_clean/core/bench.py`). Remaining: gold-standard corpora, blind expert ratings, agreement-vs-gold (κ/α/AC1, IoU/Hausdorff), bias stratification + counterfactual identity-cue tests, held-out D7 runs with baselines and F1/bootstrap intervals, live adversarial prompt-injection runs, full wall-clock run timing, confidence calibration, baselines — built on `prompt_eval`. *Closes INV-3*, operationalizes INV-5.
...
5. **Disconfirmation + adjudication over the ledger** — *FIRST SLICES DONE*: negative-case prompts include bounded claim targets by ID, BM25-style lexical/query-expanded retrieval-first source candidates, exact `candidate_id` anchors, configurable disconfirmation model routing, adversarial evidence-bound prompt stance, coverage summaries report challenged/unchallenged targets, `make bench` can score D7 against supplied adjudicated gold anchors with recall/precision Wilson intervals, and `ReviewManager` can adjudicate claim objects. Remaining for full INV-2/INV-6/INV-10: semantic/embedding/adversarial retrieval, validated model/provider policy, held-out D7 gold-set evaluation with baselines and F1/bootstrap intervals, browser-native review, relationship/negative-case review UX, and expert adjudication protocol.

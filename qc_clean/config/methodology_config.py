...
class MethodologyConfigManager:
    """Manages methodology configurations"""
    
    def __init__(self, config_base_path: Optional[Path] = None):
        """Initialize configuration manager"""
        if config_base_path is None:
            # Default to project config directory
            self.config_base_path = Path(__file__).parent.parent.parent.parent / 'config' / 'methodology_configs'
        else:
            self.config_base_path = config_base_path
        
        logger.info(f"MethodologyConfigManager initialized with base path: {self.config_base_path}")
    
    def load_config(self, methodology: str, config_name: Optional[str] = None) -> GroundedTheoryConfig:
        """
        Load methodology configuration with validation
        
        Args:
            methodology: Name of the methodology (e.g., 'grounded_theory')
            config_name: Specific config variant (defaults to methodology name)
            
        Returns:
            GroundedTheoryConfig: Loaded and validated configuration
        """
        try:
            if config_name is None:
                config_name = methodology
            
            config_path = self.config_base_path / f"{config_name}.yaml"
            
            if methodology == 'grounded_theory':
                config = GroundedTheoryConfig.from_yaml(config_path)
                logger.info(f"Loaded {methodology} configuration: {config.coding_depth} depth, "
                           f"{len(config.report_formats)} report formats")
                return config
            else:
                raise ValueError(f"Unsupported methodology: {methodology}")
                
        except Exception as e:
            logger.error(f"Failed to load {methodology} configuration: {e}")
            raise
    
    def load_config_from_path(self, config_path: Path) -> GroundedTheoryConfig:
        """Load configuration from specific path"""
        try:
            config = GroundedTheoryConfig.from_yaml(config_path)
            logger.info(f"Loaded configuration from {config_path}: {config.methodology}")
            return config
        except Exception as e:
            logger.error(f"Failed to load configuration from {config_path}: {e}")
            raise
    ...
    def list_available_configs(self) -> List[str]:
        ...
        try:
            ...
            yaml_files = list(self.config_base_path.glob("*.yaml"))
            config_names = [f.stem for f in yaml_files]
            logger.info(f"Found {len(config_names)} configuration files: {config_names}")
            return config_names
    ...
    def create_default_config(self, methodology: str, config_name: str) -> GroundedTheoryConfig:
        """Create default configuration for methodology"""
        if methodology == 'grounded_theory':
            return GroundedTheoryConfig(
                methodology="grounded_theory",
                theoretical_sensitivity="high",
                coding_depth="comprehensive",
                memo_generation_frequency="each_phase",
                report_formats=["academic_report", "executive_summary"],
                ...
                temperature=0.0,
                max_tokens=None,
                model_preference="gemini/gemini-2.5-flash"
            )
        else:
            raise ValueError(f"Unsupported methodology: {methodology}")
    
    def save_config(self, config: GroundedTheoryConfig, config_name: str) -> Path:

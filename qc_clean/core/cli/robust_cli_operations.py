...
class RobustCLIOperations:
    ...
    def __init__(self, config=None):
        ...
        self.extractor_name = 'semantic'  # Default extractor
    
    def set_extractor(self, extractor_name: str):
        """Set the extractor to use for analysis"""
        self.extractor_name = extractor_name
        logger.info(f"Extractor set to: {extractor_name}")
    ...
    def llm_handler(self):
        """Access to the LLM handler instance"""
        return self._llm_handler
    
    async def _initialize_neo4j(self) -> bool:
        """Initialize Neo4j connection with fail-fast Docker validation"""
        try:
            # FAIL-FAST: Check Docker dependencies before attempting connection
            ...
            # Initialize real Neo4j manager with correct credentials
            from ..data.neo4j_manager import EnhancedNeo4jManager
            self._neo4j_manager = EnhancedNeo4jManager(username='neo4j', password='password')  # Container credentials
            await self._neo4j_manager.connect()
            
            # Verify connection works
            await self._neo4j_manager.test_connection()
            logger.info("Neo4j connection established and tested successfully")
            return True
        ...
        try:
            if file_path.suffix.lower() == '.json':
                ...
                return {
                    ...
                    'text': text_content,           # Keep for backward compatibility
                    'content': text_content,        # Add for extractor compatibility
    ...
    async def robust_extract_data(self, interview_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Use plugin system for extractor selection
            from ...plugins.extractors import get_extractor_plugin
            
            # Get the specified extractor plugin
            extractor_plugin = get_extractor_plugin(self.extractor_name)
            if not extractor_plugin:
                # Fallback to default extraction system
                logger.warning(f"Extractor '{self.extractor_name}' not found, using CodeFirstExtractor")
                ...
                # Use the plugin extractor
                logger.info(f"Using extractor plugin: {extractor_plugin.get_name()}")
                
                # Initialize the plugin if it has LLM support
                if hasattr(extractor_plugin, 'llm_handler') and self._llm_handler:
                    extractor_plugin.llm_handler = self._llm_handler
                
                # Extract using plugin
                result = await extractor_plugin.extract_full_interview(
                    interview_data.get('text', ''),
                    interview_data.get('interview_id', 'unknown')
                )

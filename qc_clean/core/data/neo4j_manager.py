...
class EnhancedNeo4jManager:
    ...
    def __init__(self, uri: str = None, username: str = None, password: str = None):
        ...
        self.username = username if username is not None else (os.getenv("NEO4J_USERNAME") or "neo4j")
        self.password = password if password is not None else (os.getenv("NEO4J_PASSWORD") or "password")
        self.driver = None
        
        # Debug logging
        logger.info(f"Neo4j config - URI: {self.uri}, Username: {self.username}, Password: {'***' if self.password else None}")
        logger.debug(f"Auth enabled: {bool(self.username and self.password)}, Username type: {type(self.username)}, Password length: {len(self.password) if self.password else 0}")
    ...
    async def test_connection(self) -> bool:
        """Test Neo4j connection with fail-fast validation"""
        if not self.driver:
        ...
        try:
            async with self.driver.session() as session:
                ...
                if record and record["test"] == 1:
                    ...
                    raise RuntimeError("Neo4j connection test failed - unexpected result")
        except Exception as e:
            logger.error(f"Neo4j connection test failed: {e}")
            raise RuntimeError(f"Neo4j connection test failed: {e}")
    ...
    async def create_entity(self, entity: EntityNode) -> str:
        ...
        query = f"""
        MERGE (e:{labels_str} {{id: $properties.id}})
        SET e = $properties
        RETURN e.id as id
        """
    ...
    async def create_quote_node(self, quote_data: Dict[str, Any]) -> str:
        ...
        query = """
        MERGE (q:Quote {id: $properties.id})
        SET q = $properties
        RETURN q.id as id
        """

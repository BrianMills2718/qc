#!/usr/bin/env python3
"""Preflight D7 retrieval comparison inputs against a registered protocol."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run D7 comparison preflight and emit machine-readable JSON."""
    parser = argparse.ArgumentParser(
        description="Preflight D7 retrieval comparison inputs against a protocol"
    )
    parser.add_argument("protocol", help="Path to a schema_version=1 D7 comparison protocol JSON")
    parser.add_argument("gold", help="Path to a schema_version=1 D7 gold package JSON")
    parser.add_argument(
        "predictions",
        nargs="+",
        help="One or more D7 retrieval prediction package JSON files",
    )
    ...
    try:
        ...
        report = D7ComparisonPreflightReport(
            schema_version=1,
            ...
            baseline_count=0,
            errors=[D7ComparisonPreflightError(field="inputs", message=str(exc))],
...
def _load_json(path: Path, *, label: str) -> Any:
    ...
    try:
        ...
        raise ValueError(f"D7 comparison {label} file '{path}' could not be read: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise ValueError(f"D7 comparison {label} file '{path}' is not valid JSON: {exc}") from exc
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise ValueError(f"D7 comparison prediction file '{path}' could not be hashed: {exc}") from exc

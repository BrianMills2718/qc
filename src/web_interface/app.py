...
# Import the automation system components - use CLI-compatible manager
...
import os
web_interface_dir = os.path.dirname(__file__)
...
async def get_neo4j_manager():
    """Get Neo4j manager instance compatible with CLI system"""
    global neo4j_manager
    if neo4j_manager is None:
        # Use CLI-compatible Neo4j manager (mock implementation)
        neo4j_manager = EnhancedNeo4jManager(
            uri="bolt://localhost:7687",
            user="neo4j",  # CLI manager uses 'user', not 'username'
